...
#define RG_HERE_FILE_NAME "graph/med/texture/font"
...
public:
	...
	void centering() {
		setPos({ (pos.x - getSizeWord() * scale) / (RG_SYS_WINDOWSdow_Size_Standart * 2.),pos.y,pos.z });
	}
	...
	void draw() {
		...
		uint symbol = 0;
		rgTextureOn();
		//rgBindTexture(RG_Atlas_StandartPtr->texture);
		...
		for (uint i = 0; i < word.size();i++) {
			...
			*/
			...
			if(gui)
			{
				...
				//rgTranslatef(((pos.x*guiSize  + posTemp.x*scale/2.))/RG_SYS_WINDOWSdow_Size_Standart, (((posTemp.y)+pos.y*guiSize+(-characters[symbol].size.y +characters[symbol].bias.y)) - (RG_SIZEFONT*scale+13))/RG_SYS_WINDOWSdow_Size_Standart, (pos.z)*guiSize,temp);

				rgTranslatef(pos.x*scale+((posTemp.x*scale))/RG_SYS_WINDOWSdow_Size_Standart, pos.y*scale+(((-posTemp.y*scale*2)+(-characters[symbol].size.y +characters[symbol].bias.y)) - (RG_SIZEFONT*scale))/RG_SYS_WINDOWSdow_Size_Standart, (pos.z)*guiSize,temp);


				//rgLoadMatrixf(IdentityMat,rg_viewMat);
				rgScalef(scale/(RG_SYS_WINDOWSdow_Size_Standart/2.),scale/(RG_SYS_WINDOWSdow_Size_Standart/2.),1,temp);
				//rgTranslatef(pos.x+(posTemp.x)/RG_SYS_WINDOWSdow_Size_Standart, pos.y+(((posTemp.y)+(-characters[symbol].size.y +characters[symbol].bias.y)) - (RG_SIZEFONT))/RG_SYS_WINDOWSdow_Size_Standart, (pos.z),temp);

			}
			else
			{
				rgTranslatef((pos.x + posTemp.x)/RG_SYS_WINDOWSdow_Size_Standart, (pos.y+posTemp.y)/RG_SYS_WINDOWSdow_Size_Standart, pos.z,temp);
				rgScalef(scale/RG_SYS_WINDOWSdow_Size_Standart,scale/RG_SYS_WINDOWSdow_Size_Standart,1,temp);
				RG_Camera_StandartPtr->loadMatrix();
			}*/
			...
			rgTexCoord2f(
				(characters[symbol].posTex.x) / (double)(RG_Atlas_StandartPtr->size.x),
				(characters[symbol].posTex.y + characters[symbol].size.y) / (double)(RG_Atlas_StandartPtr->size.y)
			);
			rgVertex2f(0, characters[symbol].bias.y - characters[symbol].size.y);

			rgTexCoord2f(
				(characters[symbol].posTex.x + characters[symbol].size.x) / (double)(RG_Atlas_StandartPtr->size.x),
				(characters[symbol].posTex.y + characters[symbol].size.y) / (double)(RG_Atlas_StandartPtr->size.y)
			);
			...
			rgTexCoord2f(
				(characters[symbol].posTex.x + characters[symbol].size.x) / (double)(RG_Atlas_StandartPtr->size.x),
				(characters[symbol].posTex.y) / (double)(RG_Atlas_StandartPtr->size.y)
			);
	...
	void update() {
		...
		for (uint i = 0; i < word.size();i++) {
			...
			if (gui) {
				rgTranslatef(pos.x + ((posTemp.x * scale)) / RG_SYS_WINDOWSdow_Size_Standart, pos.y + (((-posTemp.y * scale * 2) + (-characters[temp].bias.y + characters[temp].bias.y)) - (RG_SIZEFONT * scale)) / RG_SYS_WINDOWSdow_Size_Standart, (pos.z) * guiSize, matrixs[i]);

				rgScalef(scale / (RG_SYS_WINDOWSdow_Size_Standart / 2.), scale / (RG_SYS_WINDOWSdow_Size_Standart / 2.), 1, matrixs[i]);


			} else {
				rgTranslatef((pos.x + posTemp.x) / RG_SYS_WINDOWSdow_Size_Standart, (pos.y + posTemp.y) / RG_SYS_WINDOWSdow_Size_Standart, pos.z, matrixs[i]);
				rgScalef(scale / RG_SYS_WINDOWSdow_Size_Standart, scale / RG_SYS_WINDOWSdow_Size_Standart, 1, matrixs[i]);
				//RG_Camera_StandartPtr->loadMatrix();			
			}
...
void RG_PrepareFont() {
	...
	POINT2D<uint>pen;
	RG_Array<uint8>texture(size.x * size.y);
	characters.resize(countChar);
