#pragma once

class RG_Atlas{
  RG_Texture atlas;

  RG_Array<RG_Texture> Textures;

  int texture = -1;
  int textureMSAA = -1;
  POINT2D<uint>size;

  void Prepare(RG_Array<RG_Texture>&texs){
    POINT2D<int>max_size;
    POINT2D<int>max_one_size;

    for(int i = 0; i<texs.size();i++){
    	max_size.x+=texs[i].texture.size(0);
    	max_size.y+=texs[i].texture.size(1);
    	if(texs[i].texture.size(0)>max_one_size.x) max_one_size.x = texs[i].texture.size(0);
    	if(texs[i].texture.size(1)>max_one_size.y) max_one_size.y = texs[i].texture.size(1);
    }
    POINT2D<int>end_size;
    while(end_size.x<max_size.x){
    	end_size.x+=1024;
    }
    	//end_size.x-=1024;

    while(end_size.y<max_size.y){
    	end_size.y+=1024;
    }
    //end_size.y-=1024;

    //RG_Matrix<int>end_texture(end_size.x,end_size.y);
    atl.textureRaw.resize(end_size.x,end_size.y);
    atl.size.x = end_size.x/4.;
    atl.size.y = end_size.y;

    uint8 cnt = 4;
    //end_texture.place({0,0},texs[0].texture);
    POINT2D<int>pen;
    POINT2D<int>max;
     
    for(int i = 0; i<texs.size();i++){AtlasTexture &texture = texs[i];
    	while(pen.x+texture.texture.size(0)>end_size.x){
    		if(pen.y>end_size.y){cout<<end_size.y<<" "<<texture.texture.size(1)<<endl;exit(-902397812);};
    		pen.x = 0;
    		pen.y += max.y+1;
    		max.y = 0;
    	}
    	//cout<<i<<endl;
    	atl.textureRaw.place({pen.x,pen.y},texs[i].texture);

    	texture.pos.x = pen.x/4;
    	texture.pos.y = pen.y;
    	pen.x+=texs[i].texture.size(0);
    	max.y = rg_max(texs[i].texture.size(1),max.y);



    }
    //cout<<"end\n";
    //stbi_write_png("atlas.png", end_size.x/cnt, end_size.y, cnt, atl.textureRaw.get_mat(), end_size.x);
    //cout<<"atlas ready\n";
    glGenTextures(1,&atl.texture);

    //rgBindTexture(atl.texture);

    glBindTexture(GL_TEXTURE_2D, atl.texture);

    		/*glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_S,GL_CLAMP);
    		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_T,GL_CLAMP);*/

    		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_S,GL_CLAMP_TO_EDGE);
    		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_T,GL_CLAMP_TO_EDGE);
    		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_R,GL_CLAMP_TO_EDGE);

    		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
    		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    		glTexParameteri(GL_TEXTURE_2D, GL_GENERATE_MIPMAP, GL_TRUE);

    		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, atl.size.x, atl.size.y,
    									0, GL_RGBA, GL_UNSIGNED_BYTE, atl.textureRaw.get_mat());

    //cout<<"ALL!\n";

  }
}RG_Atlas_Standart;

class RG_Mods{
  static bool textures = false;

public:
  static void Texture_On(){
    if(textures){
      RG_Debug::addl(RG_LOG_LOCK_INFO,"Atlas already on");
      return;
    };
    if(RG_Atlas_Standart.texture==-1){
      RG_Debug::addl(RG_LOG_LOCK_CRITICAL,"Atlas not prepare");
    }
    textures = 1;
  	rgEnableClientState(RG_TEXTURE_COORD_ARRAY);
  	rgBindTexture(RG_Atlas_Standart.texture);
  }

  static void Texture_Off(){
    if(!textures){
      RG_Debug::addl(RG_LOG_LOCK_INFO,"Atlas already off");
      return;
    }
    textures = 0;
  	rgDisableClientState(RG_TEXTURE_COORD_ARRAY);
  	glBindBuffer(GL_ARRAY_BUFFER,0);
  	rgBindTexture(0);
  }
}


