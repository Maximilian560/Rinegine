...
#define RG_HERE_FILE_NAME "graph/context"
...
// 			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, RG_SYS_WINDOWSdow_Standart->set().resolution.x, RG_SYS_WINDOWSdow_Standart->set().resolution.y, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
...
// 				RG_Debug::addl(RG_LOG_LOCK_WARNING, "Framebuffer is not complete!");
...
namespace RG_Events {
	...
	void Update(bool clear = true, RG_Timer& timer = *RG_Timer_StandartPtr) {
		/*if(!AlredyUpdate){
			glfwSwapBuffers(RG_SYS_WINDOWSdow_Standart->win());
	...
	/*void Update(){
	 RG_Debug::addl(RG_LOG_LOCK_WARNING,"RG_Events::Update() ")
	 glfwSwapBuffers(RG_SYS_WINDOWSdow_Standart->win());
	 //sizeWindowChange = false;
	 WindowWasChange = false;
	 FileWasDropped = false;
	...
	namespace Callback {//Calback
		void GLFW_Error(int err, const char* description) {
			RG_Debug::addl(RG_LOG_LOCK_ERROR, "GLFW_Error: " + to_string(err) + ", " + string(description));
		}

		void Keys(GLFWwindow* win, int key, int scancode, int action, int mods) {
			if (key == GLFW_KEY_F11 && action == GLFW_PRESS) {
				RG_SYS_WINDOWSdow_Standart->Active_Fullscreen();
				glfwSetCursorPos(RG_SYS_WINDOWSdow_Standart->win(), RG_SYS_WINDOWSdow_Standart->set().resolution.x / 2., RG_SYS_WINDOWSdow_Standart->set().resolution.y / 2.);
				//if(RG_CursorFix)glfwSetCursorPos(window,WindowSize.x/2.,WindowSize.y/2.);

			}
			if (RG_KEYS[GLFW_KEY_LEFT_CONTROL] && key == GLFW_KEY_V) {
				const char* tempa = glfwGetClipboardString(RG_SYS_WINDOWSdow_Standart->win());
				if (tempa != nullptr) {
					Clipboard = tempa;
		...
		void ViewPort(GLFWwindow* window, int x, int y) {
			RG_SYS_WINDOWSdow_Standart->set().resolution.x = x;
			RG_SYS_WINDOWSdow_Standart->set().resolution.y = y;
			RG_SYS_WINDOWSdow_Standart->set().winedit.quotx = RG_SYS_WINDOWSdow_Standart->set().resolution.x / RG_SYS_WINDOWSdow_Size_Standart;
			RG_SYS_WINDOWSdow_Standart->set().winedit.quoty = RG_SYS_WINDOWSdow_Standart->set().resolution.y / RG_SYS_WINDOWSdow_Size_Standart;
			RG_SYS_WINDOWSdow_Standart->set().winedit.difx = RG_SYS_WINDOWSdow_Standart->set().resolution.x - RG_SYS_WINDOWSdow_Size_Standart;
			RG_SYS_WINDOWSdow_Standart->set().winedit.dify = RG_SYS_WINDOWSdow_Standart->set().resolution.y - RG_SYS_WINDOWSdow_Size_Standart;
			glViewport(0, 0, x, y);
		...
		namespace Set {//Set
			...
			void Standart(RG_SYS_WINDOWSdow& win) {
				Standart(win.win());
			}
			void Keys(void(&Event)(GLFWwindow*, int, int, int, int)) {
				glfwSetKeyCallback(RG_SYS_WINDOWSdow_Standart->win(), Event);
			}
			void ViewPort(void(&Event)(GLFWwindow*, int, int)) {
				glfwSetFramebufferSizeCallback(RG_SYS_WINDOWSdow_Standart->win(), Event);
			}
			void Mouse(void(&Event)(GLFWwindow*, int, int, int)) {
				glfwSetMouseButtonCallback(RG_SYS_WINDOWSdow_Standart->win(), Event);
			}
			void Scroll(void(&Event)(GLFWwindow*, double, double)) {
				glfwSetScrollCallback(RG_SYS_WINDOWSdow_Standart->win(), Event);
			}
			void DragNDrop(void(&Event)(GLFWwindow*, int, const char**)) {
				glfwSetDropCallback(RG_SYS_WINDOWSdow_Standart->win(), Event);
			}
			void WindowPos(void(&Event)(GLFWwindow*, int, int)) {
				glfwSetWindowPosCallback(RG_SYS_WINDOWSdow_Standart->win(), Event);
			}
...
void RG_TestGLError() {
	int glError = 0;
	RG_Debug::addl(RG_LOG_LOCK_INFO, "Start test GL error");
	while ((glError = glGetError()) != GL_NO_ERROR) {
		switch (glError) {
			case GL_NO_ERROR:						RG_Debug::addl(RG_LOG_LOCK_ERROR, "GL_NO_ERROR");break;
			case GL_INVALID_ENUM:				RG_Debug::addl(RG_LOG_LOCK_ERROR, "GL_INVALID_ENUM");break;
			case GL_INVALID_VALUE:			RG_Debug::addl(RG_LOG_LOCK_ERROR, "GL_INVALID_VALUE");break;
			case GL_INVALID_OPERATION:	RG_Debug::addl(RG_LOG_LOCK_ERROR, "GL_INVALID_OPERATION");break;
			case GL_STACK_OVERFLOW:			RG_Debug::addl(RG_LOG_LOCK_ERROR, "GL_STACK_OVERFLOW");break;
			case GL_STACK_UNDERFLOW:		RG_Debug::addl(RG_LOG_LOCK_ERROR, "GL_STACK_UNDERFLOW");break;
			case GL_OUT_OF_MEMORY:			RG_Debug::addl(RG_LOG_LOCK_ERROR, "GL_OUT_OF_MEMORY");break;
			default:										RG_Debug::addl(RG_LOG_LOCK_ERROR, "Unknown error");break;
		};
	}
	RG_Debug::addl(RG_LOG_LOCK_INFO, "Stop test GL error");
}
