...
/*int RG_Action(RG_Object& obj,bool sticky = false){
	...
	if(obj.gui_type == RG_GUI_LINK_LEFT_CENTER) {HPos.x = 0; HPos.y = RG_SYS_WINDOWSdow_Standart->set().resolution.y/2.;}
	if(obj.gui_type == RG_GUI_LINK_LEFT_BOTTOM) {HPos.x = 0; HPos.y = RG_SYS_WINDOWSdow_Standart->set().resolution.y;}

	if(obj.gui_type == RG_GUI_LINK_CENTER_TOP) {HPos.x = RG_SYS_WINDOWSdow_Standart->set().resolution.x/2.; HPos.y = 0;}
	if(obj.gui_type == RG_GUI_LINK_CENTER_CENTER) {HPos.x = RG_SYS_WINDOWSdow_Standart->set().resolution.x/2.; HPos.y = RG_SYS_WINDOWSdow_Standart->set().resolution.y/2.;}
	if(obj.gui_type == RG_GUI_LINK_CENTER_BOTTOM) {HPos.x = RG_SYS_WINDOWSdow_Standart->set().resolution.x/2.; HPos.y = RG_SYS_WINDOWSdow_Standart->set().resolution.y;}

	if(obj.gui_type == RG_GUI_LINK_RIGHT_TOP) {HPos.x = RG_SYS_WINDOWSdow_Standart->set().resolution.x; HPos.y = 0;}
	if(obj.gui_type == RG_GUI_LINK_RIGHT_CENTER) {HPos.x = RG_SYS_WINDOWSdow_Standart->set().resolution.x; HPos.y = RG_SYS_WINDOWSdow_Standart->set().resolution.y/2.;}
	if(obj.gui_type == RG_GUI_LINK_RIGHT_BOTTOM) {HPos.x = RG_SYS_WINDOWSdow_Standart->set().resolution.x; HPos.y = RG_SYS_WINDOWSdow_Standart->set().resolution.y;}


	POINT3D <double>size;

	size.x=obj.size.x * RG_SYS_WINDOWSdow_Standart->set().resolution.x* obj.scale *((float)RG_SYS_WINDOWSdow_Standart->set().resolution.y / RG_SYS_WINDOWSdow_Standart->set().resolution.x);
	size.y=obj.size.y * RG_SYS_WINDOWSdow_Standart->set().resolution.y* obj.scale ;
	...
	{
		(HPos.x+(obj.pos.x/2.*RG_SYS_WINDOWSdow_Standart->set().resolution.x)*((float)RG_SYS_WINDOWSdow_Standart->set().resolution.y / RG_SYS_WINDOWSdow_Standart->set().resolution.x)-(size.x/2)),
		(HPos.y+(-obj.pos.y/2. * RG_SYS_WINDOWSdow_Standart->set().resolution.y)) - (size.y/2.)
	};
...
void RG_CreateText(RG_Text& txt, wstring text, POINT3D <double> pos, double sc = 0, bool gui = false, LINK_GUI_TYPE typeGui = RG_GUI_LINK_NONE, bool center = false) {
	...
	if (center) {
		txt.setPos({ (pos.x - txt.getSizeWord() * sc) / (RG_SYS_WINDOWSdow_Size_Standart * 2.),pos.y,pos.z });
	} else {
...
/*void RG_CreateText(RG_Text& txt, wstring text, RG_Object& obj,double sc = 1)
{
	...
	txt.setPos({obj.pos.x-((txt.getSizeWord()* ((obj.scale*sc)/2.))/(RG_SYS_WINDOWSdow_Size_Standart )),obj.pos.y,obj.pos.z});
...
*/
struct RG_Standart {
	inline static RG_SYS_WINDOWSdow*& Window = RG_SYS_WINDOWSdow_Standart;
	...
	void operator=(const RG_Standart& in) {
		...
		RG_SYS_WINDOWSdow_Standart = in.Window;
