...
#define RG_HERE_FILE_NAME "graph/high/gen-chunk"
...
namespace RG {
  ...
  public:
    void init(POINT3D<uint>in) {
      _size = in;
      _max_status = in.x * in.y * in.z * 1 + 3;
    }
    void init(uint x, uint y, uint z) {
      _size = { x,y,z };
      _max_status = x * y * z * 1 + 3;
    }
    ...
    uint get_status() const { return *_status_ptr; }
    uint *get_status_ptr() const { return _status_ptr; }
    // void set_mutex() {
    //   _mut_ptr = &_mut;
    // }
    ...
    void gl_texture_gen() {
      if (GL_TEXTURE_BUFFER_CHUNK.empty()) {
        ...
        if (textures[0] < 0)
          RG_LOG_LOCK_ERROR("Texture Grass Not Found");
        if (textures[1] < 0)
          RG_LOG_LOCK_ERROR("Texture Dirt Not Found");
        if (textures[2] < 0)
          RG_LOG_LOCK_ERROR("Texture Stone Not Found");
        ...
        for (int y = 0; y < _size.y; y++) {
          for (int z = 0; z < _size.z; z++) {
            for (int x = 0; x < _size.x; x++) {
              ...
              if (_mut_ptr)
                _mut_ptr->lock();
              (*_status_ptr)++;//+_size.y*_size.x*_size.z
              if (_mut_ptr)
                _mut_ptr->unlock();
            }
          }
