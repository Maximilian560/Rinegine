      ...
      if (cnt < 4) {
        ...
        if (cnt == 3) {
          if ((temp2 = stbi_load(tempOUT.c_str(), &size.x, &size.y, &cnt, 4)) != nullptr) {
            RG_Debug::addl(RG_LOG_LOCK_WARNING, "Texture: '" + path + "' has 3 color channels, but found a '" + path + ".png' file that is similar to the one generated by this warning. If not, rename one of these files");
            raw.fill(size.x * cnt, size.y, temp2);
          }
          else {
            RG_Debug::addl(RG_LOG_LOCK_WARNING, "Texture: '" + path + "' has 3 color channels and will be converted to '" + path + ".png' with 4 channels, please use it next time");
          ...
          RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Texture: '" + path + "'' cnt is " + to_string(cnt) + "and it not support!");
        }
