...
public:
  RG_Material(int prog){
    if(prog<0&&RG_D_W_L>=2)RG_Debug::addl(RG_LOG_LOCK_WARNING,"Material initialized with a negative OpenGL shader value: "+to_string(prog));
    gl_shader_prog = prog;
  }
  ...
  ~RG_Material(){
    if(gl_shader_prog!=RG_STANDART_SHADER_PROG){
      ...
      if(RG_D_W_L>=4)RG_Debug::addl(RG_LOG_LOCK_WARNING,"The material has been destroyed but the shader program is still initialized and is not equal to the standard shader program. Shader program handle: "+to_string(gl_shader_prog));
    }
  }
