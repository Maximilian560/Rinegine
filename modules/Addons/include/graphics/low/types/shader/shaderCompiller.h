...
#define RG_HERE_FILE_NAME "graph/low/types/shader-compile"
...
void ShaderCompiller(char* frag, char* vert, uint prog)//
{
	...
	if (!fsok) {
		...
		log += temp;
		//DEBUG
		RG_LOG_LOCK_ERROR("Fragment shader compilation error:\n" + (string)temp);
		//DEBUG
	}
	if (!fpok) {
		...
		string tempa = temp;
		if (tempa.size() <= 0) {
			RG_LOG_LOCK_ERROR("Fragment shader linking error");
		}
		else {
			RG_LOG_LOCK_ERROR("Fragment shader linking error:\n" + (string)temp);
		};
	...
	if (!vpok) {
		...
		string tempa = temp;
		if (tempa.size() <= 0) {
			RG_LOG_LOCK_ERROR("Vertex shader linking error");
		}
		else {
			RG_LOG_LOCK_ERROR("Vertex shader linking error:\n" + (string)temp);
		};
