...
{
	RG_Debug::addl(RG_LOG_LOCK_INFO, "Compiling shaders");
	...
	if (!fsok) {
		...
		log += temp;
		//DEBUG
		RG_Debug::addl(RG_LOG_LOCK_ERROR, "Fragment shader compilation error:\n" + (string)temp);
		//DEBUG
	}
	if (!fpok) {
		...
		string tempa = temp;
		if (tempa.size() <= 0) {
			RG_Debug::addl(RG_LOG_LOCK_ERROR, "Fragment shader linking error");
		}
		else {
			RG_Debug::addl(RG_LOG_LOCK_ERROR, "Fragment shader linking error:\n" + (string)temp);
		};
	...
	if (!vsok) {
		...
		log += temp;
		//DEBUG
		RG_Debug::addl(RG_LOG_LOCK_ERROR, "Vertex shader compilation error:\n" + (string)temp);
		//DEBUG
	}
	if (!vpok) {
		...
		string tempa = temp;
		if (tempa.size() <= 0) {
			RG_Debug::addl(RG_LOG_LOCK_ERROR, "Vertex shader linking error");
		}
		else {
			RG_Debug::addl(RG_LOG_LOCK_ERROR, "Vertex shader linking error:\n" + (string)temp);
		};
