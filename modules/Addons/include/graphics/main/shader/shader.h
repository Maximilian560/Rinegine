...
/*#ifdef RG_EXP_SHD//TODO !!!
		if(RG_OpenGLVersion <= 460 && RG_OpenGLVersion >= 330){
			...
			RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "OpenGL " + RG_OpenGLVersionSTR + " with experimental shader not support");
		}
#else */
		RG_Debug::addl(RG_LOG_LOCK_INFO, "OpenGL: " + RG_OpenGLVersionSTR);
		if(RG_OpenGLVersion <= 460 && RG_OpenGLVersion >= 330){
			init(RG_StandartFragmentShader330, RG_StandartVertexShader330);
		}elif(RG_OpenGLVersion > 460){
			// init(RG_StandartFragmentShader330, RG_StandartVertexShader330);
			RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Where did you get OpenGL version " + RG_OpenGLVersionSTR + " ?");
		}elif(RG_OpenGLVersion < 330 && RG_OpenGLVersion >= 220){
			init(RG_StandartFragmentShader220, RG_StandartVertexShader220);
		}else{
			RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "OpenGL " + RG_OpenGLVersionSTR + " not support");
		}
//#endif
	}
	/*INIT = true;
