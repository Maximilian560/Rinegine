...
public:
	...
	void move(POINT2D<int>in) {
		// RG_Debug::addl(RG_LOG_LOCK_INFO,"move to "+ std::to_string(in.x) + " " + std::to_string(in.y));
	...
	void init(RG_SYS_WINDOWSdow_Settings& set) {
		...
		switch (settings.Wtype) {
			case RG_SYS_WINDOWSdowed:
				...
				// if (!window) {
				// 	RG_Debug::addl(RG_LOG_LOCK_ERROR,"Critical, window not created");
				// 	glfwTerminate();
				// 	throw(RG_ERROR_WINDOWS_NO_CREATED);
				// }
				glfwSetWindowPos(window, settings.position.x, settings.position.y);
		...
		// if (is_link != RG_Current_Is_Enabled)RG_Debug::addl(RG_LOG_LOCK_WARNING, "OpenGL context is busy");
		// if (is_link)RG_Debug::addl(RG_LOG_LOCK_WARNING, "Window is already linked");
		...
		if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress)) {
			RG_Debug::addl(RG_LOG_LOCK_ERROR, "Failed to initialize GLAD Loader");
		}
		if (!gladLoadGL()) {
			RG_Debug::addl(RG_LOG_LOCK_ERROR, "Failed to initialize GLAD");
		}
		const GLubyte* TEMPCHAR = glGetString(GL_VERSION);
		int countss = 1;
		RG_OpenGLVersion = 0;
...
rgEnableClientState(RG_COLOR_ARRAY);*/
	...
	void link() {
		if (is_link != RG_Current_Is_Enabled)RG_Debug::addl(RG_LOG_LOCK_ERROR, "OpenGL context is busy");
		if (is_link)RG_Debug::addl(RG_LOG_LOCK_ERROR, "Window is already linked");
		glfwMakeContextCurrent(window);
		if (glfwGetCurrentContext() != window) {
			RG_Debug::addl(RG_LOG_LOCK_ERROR, "Error making current context"
#ifdef RG_DLL
				" in dll"
#endif
			);
			...
			glfwMakeContextCurrent(window);
			if (glfwGetCurrentContext() != window)RG_Debug::addl(RG_LOG_LOCK_ERROR, "An attempt to fix a context creation error was unsuccessful");
			if (window == nullptr)RG_Debug::addl(RG_LOG_LOCK_ERROR, "The reason was found, window == nullptr");
		}
		is_link = true;
		RG_Current_Is_Enabled = 1;
	}
	...
	void unlink() {
		if (is_link != RG_Current_Is_Enabled)RG_Debug::addl(RG_LOG_LOCK_ERROR, "OpenGL context is busy");
		if (!is_link) {
			RG_Debug::addl(RG_LOG_LOCK_ERROR, "Window is already unlinked");
		}
		glfwMakeContextCurrent(nullptr);
		is_link = false;
