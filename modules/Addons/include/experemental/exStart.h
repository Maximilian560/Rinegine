...
/*void RR_KeyEvent(GLFWwindow* win, int key, int scancode, int action, int mods)
{
	if(key == GLFW_KEY_F11&&action==GLFW_PRESS)
	{
		...
		//if(RG_CursorFix)glfwSetCursorPos(RG_SYS_WINDOWSdow_Standart->mon(),RG_SYS_WINDOWSdow_Standart->set().resolution.x/2.,RG_SYS_WINDOWSdow_Standart->set().resolution.y/2.);
	}
...
int RG_EX_Start(RG_Array<int(*)()> &funcs){
  ...
  RG_SYS_WINDOWSdow_Standart = &window;
  engine.init();
  ...
  RG_SetAllCallback(window);
 	glfwSetKeyCallback(RG_SYS_WINDOWSdow_Standart->win(), RR_KeyEvent);
