...
#define RG_HERE_FILE_NAME "add/player"

class RG_Player{
  friend class RG_World;
  RG_Entity* body;
  RG_Camera &PlayerCam = RG_Camera_StandartPtr;
...
public:
  ...
  void ConectToWorld(RG_World&world){
    RG_LOG_LOCK_INFO("WTF1");//#DELETE
    // body = RG::s_new<RG_Entity>(1);
    isConnectToWorld = true;
    connectWorld = &world;

    //rg_cout<<connectWorld->Entitys.size()<<endl;
    //rg_cout<<world.Entitys.size()<<endl;
  ...
  bool on_ground(){
    ...
    floor(-RG_Camera_StandartPtr->getPos().x/50.),
    floor(-(RG_Camera_StandartPtr->getPos().y+body->size.y)/50.),
    floor(-RG_Camera_StandartPtr->getPos().z/50.)};
    //rg_cout<<posInBlock.x<<" "<<posInBlock.y<<" "<<posInBlock.z<<endl;
    switch(isConnectToWorld){
