...
public:
  ...
  static int add(RG_Array<type>* link, int a){
    RG_Debug::addl(RG_LOG_INFO,"RG_Pointer type of '"+string(abi::__cxa_demangle(typeid(type).name(), NULL, NULL, NULL))+"' has created, size = '"+to_string(a)+"'");
    RG_Debug::addl(RG_LOG_INFO,"RG_Pointer pointers pointer = '"+to_string((long long)pointers.get()));
    pointers.resize(_size+1);
    pointers[_size].resize(a);
    RG_Debug::addl(RG_LOG_INFO,"RG_Pointer pointers pointers pointer = '"+to_string((long long)pointers[_size].get()));
    pointers[_size]._noclear = true;
    RG_Debug::addl(RG_LOG_INFO,"RG_Pointer new pointers pointer = '"+to_string((long long)pointers.get()));
  ...
  static void link(RG_Array<type>* link, int id){
    RG_Debug::addl(RG_LOG_INFO,"RG_Pointer link old pointer = '"+to_string((long long)links.get())+"'");
    RG_Debug::addl(RG_LOG_INFO,"This old pointers: ");
    for(int i = 0; i<_size;i++){
      RG_Debug::addl(RG_LOG_INFO,to_string((long long)links[i].link)+" "+to_string(links[i].arrayid));
    }
    links.push_back({link,id});
    RG_Debug::addl(RG_LOG_INFO,"This new pointers: ");
    for(int i = 0; i<_size;i++){
      RG_Debug::addl(RG_LOG_INFO,to_string((long long)links[i].link)+" "+to_string(links[i].arrayid));
    }
    RG_Debug::addl(RG_LOG_INFO,"RG_Pointer link new pointer = '"+to_string((long long)links.get())+"'");
    link->_array = pointers[id].get();
    RG_Debug::addl(RG_LOG_INFO,"RG_Pointer link link pointer = '"+to_string((long long)link)+"'");
  ...
  static void check_link_and_free(int id){
    ...
    if(temp!=2){
      ...
      RG_Debug::addl(RG_LOG_INFO,"Pointers pointer is '"+to_string((long long)pointers.get_arr())+"'");
      pointers.erase(id);
      _size--;
    }
