...
#define RG_HERE_FILE_NAME "def/types/raw_array"
/*struct rg_raw_array{
  void*arr;
  int size=0;
  void   
...
public:
  ...
  void resize(to_rvalue(uint) size){
    RG_LOG_LOCK_INFO("Raw _array resize, old size = '"+to_string(_size)+"', new size = '"+to_string(size)+"', old _array = '"+to_string((long long)_array)+"'");
    RG_LOG_LOCK_INFO("Raw _array type of '"+string(abi::__cxa_demangle(typeid(type).name(), NULL, NULL, NULL))+"'");
    if(size>0){
     if(_size==0){
          //_array = RG::s_new<type>(size);
          //RG_LOG_LOCK_INFO("Raw _array resize "+to_string(size));
          _array = RG::s_new<type>(size);
    ...
    RG_LOG_LOCK_INFO("Raw _array resize new _array = '"+to_string((long long)_array)+"'");
      //INIT = true;
      _size = size;
    }else{
  ...
  type&operator[](to_rvalue(int) i)const{
    if(_size==0)RG_LOG_LOCK_CRITICAL("Size _array = 0");
    if(i>=0){
      if(i<=_size){
        return _array[i];
      }
      else {
        RG_LOG_LOCK_CRITICAL("i > 0. Size _array loss then 'i' in operator[](uint i), i = '"+to_string(i)+"', size = '"+to_string(_size)+"'");
      }
    }
    else{
      if(int(_size)+i<_size){
        ...
        RG_LOG_LOCK_CRITICAL("i < 0. i + _array size is greater than the _array size, _array size = '"+to_string(_size)+"', i = '"+to_string(i)+"', i + _array size'"+to_string(int(_size)+i)+"'");
      }
    }
    RG_LOG_LOCK_CRITICAL("Array unknow error");
    return _array[0];
  }
  /*void operator=(to_rvalue(RG_RawArray<type>)ar){
    //resize(ar.size());
    //rg_cout<<"WTF "<<ar.size()<<endl;
    if(_size!=0){
      if(_size!=ar._size)rg_cout<<L"ВНИМАНИЕ! РАЗМЕР МАССИВА БЫЛ ИЗМЕНЁН С "<<_size<<L" НА '"<<ar._size<<L"' !!!!\n";
    }
  ...
  /*void operator=(to_rvalue(RG_RawArray<type>)in){
    ...
    RG_LOG_LOCK_WARNING("A copy of the _array has been created, which points to the same _array that is associated with the original, thus, when the original is deleted, crashes due to access to unallocated memory or garbage return are possible.");
  }*/
 ...
 void operator=(to_rvalue(RG_RawArray<type>)in){
  ...
  void clear(){
    /*if(_size!=0||INIT!=false){
      ...
      RG_LOG_LOCK_INFO("Raw_array should be delete: '"+to_string((long long)_array)+"' _size '"+to_string(_size)+"'");
    if(_size>0&&_array!=nullptr&&!_noclear){
      ...
      /*if(!stbi)
        ...
        free(_array);*/
        //RG_LOG_LOCK_INFO("Raw _array delete "+to_string((long long)_array));
      //rg_cout<<"delete "<<_array<<endl;
      RG_LOG_LOCK_INFO("RAW ARRAY CLEAR _array '"+to_string((long long)_array)+"' _size '"+to_string(_size)+"'");
      RG::s_delete(_array);
  ...
  /*void * operator new[] (size_t){
      RG_LOG_LOCK_INFO("RAW ARRAY NEW _array '"+to_string((long long)_array)+"' _size '"+to_string(_size)+"'");
      
      return 0;
   }*/
   /*void   operator delete[] (void*){
      RG_LOG_LOCK_INFO("RAW ARRAY DELETE _array '"+to_string((long long)_array)+"' _size '"+to_string(_size)+"'");
   }*/
  ~RG_RawArray(){
    clear();
  }
