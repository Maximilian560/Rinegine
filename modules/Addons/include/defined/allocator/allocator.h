...
#define RG_HERE_FILE_NAME "def/alloc"
#include <type_traits>
template<class type>
class RG_Array;
namespace RG {
  namespace Lock {
    ...
    //       if (newmem == nullptr)RG_LOG_LOCK_CRITICAL("MEMORY ALLOCATE ERROR");
    //       newmem[0] = 'R';
    //       newmem[1] = 'G';
    ...
    //       //RG_LOG_LOCK_MEM("Some type size is '"+to_string(sizeof(type))+"' has already taken up '"+to_string(Lock::CountPointers<type>::size*sizeof(type))+"' bytes");
    //       Lock::MemUsed += size * sizeof(type) + sizeof(uint) + sizeof(char) * 2;
    //       /*#ifdef RG_MEMLOG
    //       RG_LOG_LOCK_MEM("Memory occupied by the program: '"+to_string(Lock::MemUsed)+"' bytes or '"+to_string(Lock::MemUsed/1024./1024.)+" MB'");
    //       #else
    //       RG_LOG_LOCK_MEM("Memory occupied by the program: '"+to_string(Lock::MemUsed)+"' bytes or '"+to_string(Lock::MemUsed/1024./1024.)+" MB'",false);
    //       #endif*/

    //       RG_LOG_LOCK_MEM("Memory allocate: '" + to_string(size * sizeof(type) + sizeof(uint) + sizeof(char) * 2) + "' bytes or '" + to_string((size * sizeof(type) + sizeof(uint) + sizeof(char) * 2) / 1024. / 1024.) + " MB'");

    // #ifdef RG_MEM_LIMIT
    //       if (Lock::MemUsed >= RG_MEM_LIMIT)RG_LOG_LOCK_CRITICAL("Memory limit exceeded");
    // #endif
    //       return out;
    //     }
  ...
  inline type* s_new(int size) {
    ...
    for (int i = 1; i <= 30 && !newmem; i++) {
      ...
      if (newmem == nullptr) RG_LOG_LOCK_ERROR("Memory allocate error, retry: " + to_string(i) + "/" + to_string(30));
    }
    if (newmem == nullptr) RG_LOG_LOCK_CRITICAL("MEMORY ALLOCATE ERROR");
    ...
    // RG_LOG_LOCK_MEM("Memory allocate: '" +
    ...
    //   " MB'");
    RG_LOG_LOCK_MEM("Mem aloc: " + to_string(size * sizeof(type) + sizeof(uint) + sizeof(char) * 2) + "b, type: " + to_string(sizeof(type)));
...
#ifdef RG_MEM_LIMIT
    if (Lock:: >= RG_MEM_LIMIT) RG_LOG_LOCK_CRITICAL("Memory limit exceeded");
#endif
  ...
  inline char s_print(wstring* in) {
    int temp_size = s_get_size(in);
    for (int i = 0; i < temp_size; i++) {
      // rg_cout << "L\"";
      rg_cout << rg_to_string(in[i]) << ", ";
  ...
  inline void s_delete(type*& in) {
    ...
    RG_LOG_LOCK_MEM("Memory has been cleared: '"+to_string(s_get_size(in)*sizeof(type))+" bytes");
  ...
  typename std::enable_if<std::is_class<type>::value, void>::type
    ...
    if (s_memtest(in)) {
      ...
      // RG_LOG_LOCK_MEM("Memory has been cleared: " + to_string(s_get_size(in) * sizeof(type) + sizeof(uint) + sizeof(char) * 2) + "b");
      RG_LOG_LOCK_MEM("Mem clean: " + to_string(s_get_size(in) * sizeof(type) + sizeof(uint) + sizeof(char) * 2) + "b, type: " + to_string(sizeof(type)));
  ...
  //     RG_LOG_LOCK_MEM("Memory has been cleared: '" + to_string(s_get_size(in) * sizeof(type) + sizeof(uint) + sizeof(char) * 2) + "' bytes");
  ...
  typename std::enable_if<!std::is_class<type>::value, void>::type
    ...
    if (s_memtest(in)) {
      ...
      Lock::CountPointers<type>::count--;
      Lock::MemUsed -= size * sizeof(type) + sizeof(uint) + sizeof(char) * 2;
      // RG_LOG_LOCK_MEM("Memory has been cleared: '" + to_string(size * sizeof(type) + sizeof(uint) + sizeof(char) * 2) + "' bytes");
      RG_LOG_LOCK_MEM("Mem clean: " + to_string(s_get_size(in) * sizeof(type) + sizeof(uint) + sizeof(char) * 2) + "b, type: " + to_string(sizeof(type)));
  ...
  inline void s_resize(type*& p, to_rvalue(int) newsize) {
    if (newsize > 0) {
      if (p != nullptr) {
        if (!s_memtest(p)) { RG_LOG_LOCK_ERROR("Memory Resize is failed, array is not RG type"); return; };
  ...
  inline void s_renew(type*& p, to_rvalue(int) newsize) {
    if (newsize > 0) {
      if (p != nullptr) {
        if (!s_memtest(p)) { RG_LOG_LOCK_ERROR("Memory Renew is failed, array is not RG type"); return; };
        int size = s_get_size(p);
        if (size != newsize) {
          s_delete<type>(p);
          p = s_new<type>(newsize);
  ...
  int s_erase(type*& p, int arrsize, int pos, int size) {// p - начала массива, arrsize - размер массива, pos - начала удаления, size - количество элементов для удаления
    RG_LOG_LOCK_WARNING("function s_erase(type*& p, int arrsize, int pos, int size) is deprecated. Please refer to the documentation");
    if (size == 0)return arrsize;
    int eracesize = size;
    int eracepos = pos;
    if (pos + size > arrsize) { RG_LOG_LOCK_ERROR("s_erase, size+pos is greater than the size of the passed array, erase pos = '" + to_string(pos) + "', erase size = '" + to_string(size) + "', array size = '" + to_string(arrsize) + "'"); }
    if (pos > arrsize) { RG_LOG_LOCK_CRITICAL("s_erase, pos is greater than the size of the passed array, erase pos = '" + to_string(pos) + "',array size = '" + to_string(arrsize) + "'"); }
    if (arrsize <= 0)RG_LOG_LOCK_CRITICAL("s_erase, array size less than or equal to 0, array size = " + to_string(arrsize) + "'");
    if (pos < 0)eracepos = arrsize + pos;
    if (size < 0) {
      if (size + pos <= 0)RG_LOG_LOCK_CRITICAL("I'm tired of writing errors, but here it's supposedly position + cleanup size is less than zero, understand it as you want. And i want to sleep.\n");
    }
    if (pos + size > arrsize)eracesize = arrsize - pos;
    if (arrsize - size <= 0) { RG_LOG_LOCK_DEBUG( "s_erase, array size less than or equal to 0"); s_delete(p); return 0; }
  ...
  int s_erase_new(type*& p, int pos, int size) {// p - начала массива, arrsize - размер массива, pos - начала удаления, size - количество элементов для удаления
    RG_LOG_LOCK_WARNING("function s_erase(type*& p, int arrsize, int pos, int size) is deprecated. Please refer to the documentation");
    if (!s_memtest(p)) { RG_LOG_LOCK_ERROR("s_erase, array is not of type RG, 'size' 0 returned"); return 0; }
