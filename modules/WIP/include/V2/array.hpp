...
#define RG_HERE_FILE_NAME "wip/array"
namespace Rinegine {
  namespace WIP {
    ...
    public:
      ...
      virtual void init(size_t nsize) {
        if (data) { RG_LOG_LOCK_ERROR(std::format("RawArray error: mem already init"));return; }
          ...
          if (MEM_CELL_TEST(data)) {
            RG_LOG_LOCK_CRITICAL("RawArray error: init failed");
          }
          _size = nsize;
        }
      }
      ...
      void resize(size_t nsize) {
        if (!data)init(nsize);
          ...
          GlobalAllocator.deallocate(head);
        }
      }
      inline void clear() {
    ...
    public:
      ...
      type& operator[](size_t index) {
        if (index > _size) {
          RG_LOG_LOCK_CRITICAL(std::format("Array operator[] error: out of range [{}>{}]", index, _size));
        }
        return data[index];
      }
    };
    ...
    void ArrayTests() {
      RG_LOG_LOCK_INFO(std::format("Start Array tests"));
      RG_LOG_LOCK_INFO(std::format("Create Array and try init by 40 bytes"));
      Array<char> test(40);
      RG_LOG_LOCK_INFO(std::format("Try read, rewrite and read again"));
      for (int i = 0; i < 40;i++) {
      ...
      RG_LOG_LOCK_INFO(std::format("Try init large mem and dealloc both"));
      Array<size_t> test2(1024 * 1024 * 1024);
      RG_LOG_LOCK_INFO(std::format("Try init"));
      for (size_t i = 0; i < 1024 * 1024 * 1024;i++) {
        test2[i] = i;
      }
      RG_LOG_LOCK_INFO(std::format("Try dealloc"));
      test.clear();
      test2.clear();
      RG_LOG_LOCK_INFO(std::format("Start Array tests"));
