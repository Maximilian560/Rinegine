  ...
  void* WIP::Lock::s_new(const size_t& size, const size_t& typesize) { // todo                     ||
    ...
    if (raw_newmem == MAP_FAILED) {
      for (int i = 1; i <= 30 && !raw_newmem; i++) {
        ...
        if (raw_newmem == MAP_FAILED)
          RG_LOG_LOCK_ERROR("Memory allocate error, retry: " + std::to_string(i) + "/" + std::to_string(30));
      }
      if (raw_newmem == MAP_FAILED)
...
#endif
  if (ptr == nullptr) { RG_LOG_CRITICAL("Allocator: fault alloc new page"); }
  else {
    // RG_LOG_DEBUG("ptr of pool: "+std::to_string((long long)ptr)+", ptr of tail: "+std::format("{x}",(long long)(MEM_TAIL*)((char*)ptr) + align - sizeof(MEM_TAIL))+", size of tail: "+std::to_string(sizeof(MEM_TAIL)));
    RG_LOG_DEBUG(std::format("ptr of pool: {:#x}, ptr of tail: {:#x}, size of tail: {:d}", (long long)ptr, (long long)(MEM_TAIL*)(((char*)ptr) + align - sizeof(MEM_TAIL)), sizeof(MEM_TAIL)));
    MEM_TAIL* tail = (MEM_TAIL*)(((char*)ptr) + (align - sizeof(MEM_TAIL)));
    tail->magic1 = RG_MAG_NUM;
    tail->magic2 = RG_MAG_NUM;
    // RG_LOG_MEM(std::string("ID ") + std::to_string(SYS_MEM_ID) + "; " + std::to_string(align) + " bytes of memory allocated (" + std::to_string(bytes) + " bytes were requested)");
    RG_LOG_MEM(std::format("ID {:d}| {:d} bytes of memory allocated ({:d} bytes were requested)", SYS_MEM_ID, align, bytes));
    ptr->size = align;
...
inline void SYS_DEL_MEM(MEM_HEAD*& in) {//[done]
  if (in != nullptr) {
    if (in->magic == RG_MAG_NUM) [[likely]] {
      // RG_LOG_MEM(std::string("ID: ") + std::to_string(in->id) + "; try deallocate");
      RG_LOG_MEM(std::format("ID: {:d}| try deallocate", in->id));
      if (RG::K::Flags::has(in->flags, MEM_FLAG::IS_USED)) [[likely]] {
        if (!RG::K::Flags::has(in->flags, MEM_FLAG::LOCKED)) [[likely]] {
          MEM_TAIL* tail = (MEM_TAIL*)(((char*)in) + ((in->size) - sizeof(MEM_TAIL)));
          if (tail->magic1 != tail->magic2 || tail->magic2 != RG_MAG_NUM) [[unlikely]] {
            RG_LOG_CRITICAL("The memory tail for identifier " + std::to_string(in->id) + " is corrupted, but the cell will still be cleared and the program will continue to work. In case of any crash, it is necessary to track the movement of memory under identifier " + std::to_string(in->id) + ".");
          }
          // RG_LOG_MEM(std::to_string(in->size) + " bytes of memory deallocated");
          RG_LOG_MEM(std::format("ID: {:d}| {:d} bytes of memory deallocated", in->id, in->size));
          RG_LOG_DEBUG(std::format("ptr of pool: {:#x}, ptr of tail: {:#x}, size of tail: {:d}", (long long)in, (long long)(MEM_TAIL*)(((char*)in) + in->size - sizeof(MEM_TAIL)), sizeof(MEM_TAIL)));
          in->flags = 0;
          in->magic = 0;
          in->size = 0;
#ifdef RG_WIN
...
#elif defined(RG_LINUX)
          // RG_LOG_WARN("Mem has not been cleared for debug!");
          munmap(in, in->size);
#else
          RG_LOG_ERROR("SYS_DEL_MEM: For what?");
#endif
          in = nullptr;
        }
        else {
          // RG_LOG_ERROR("SYS_DEL_MEM: mem " + std::to_string((long long)in) + " is locked");
          RG_LOG_ERROR(std::format("SYS_DEL_MEM: mem {:#x} is locked", ((long long)in)));
        }
      }
      else {
        // RG_LOG_ERROR("SYS_DEL_MEM: mem " + std::to_string((long long)in) + " isn't allocated");
        RG_LOG_ERROR(std::format("SYS_DEL_MEM: mem {:#x} isn't allocated", ((long long)in)));
      }
    }
    else {
      // RG_LOG_ERROR("SYS_DEL_MEM: mem " + std::to_string((long long)in) + " isn't allocated or isn't Rinegine type");
      RG_LOG_ERROR(std::format("SYS_DEL_MEM: mem {:#x} isn't allocated or isn't Rinegine type", ((long long)in)));
    }
  }
  else {
    // RG_LOG_ERROR("SYS_DEL_MEM: mem " + std::to_string((long long)in) + " is nullptr");
    RG_LOG_ERROR(std::format("SYS_DEL_MEM: mem {:#x} is nullptr", ((long long)in)));
  }
}
...
inline bool MEM_CELL_TEST(void* ptr) {
  ...
  if (head->magic == RG_MAG_NUM) {
    if ((tail->magic1 == tail->magic2) && (tail->magic1 == RG_MAG_NUM)) {
      ...
      RG_LOG_ERROR(std::format("Memory id {:d} from pool id {:d} is corrupted!", head->id, head->pool_id));
    }
    RG_LOG_ERROR("MEM_CELL_TEST Error: ptr is not rg type");
  }
  return 1;
}
...
public:
  //[constructor]
  Allocator() {
    RG_LOG_DEBUG("Create new Allocator");
    pool.init();
    RG_LOG_DEBUG(std::string("Pool is ") + std::string((pool.pool->magic == RG_MAG_NUM) ? "init" : "doesn't init"));
    if (RG::K::Flags::has(pool.pool->flags, MEM_FLAG::LOCKED)) {
      RG_LOG_WARN(std::format("Pool {:d} mem id {:d} already locked", pool.pool->pool_id, pool.pool->id));
    }
    else {
      RG::K::Flags::set(pool.pool->flags, MEM_FLAG::LOCKED);
      RG_LOG_MEM(std::format("Pool {:d} mem id {:d} now is locked", pool.pool->pool_id, pool.pool->id));
    }
  }
  void reallocate(void* ptr, size_t nsize) {
    if (MEM_CELL_TEST(ptr)) {
      RG_LOG_CRITICAL("Allocator error: reallocate get corrupted memory");
    }
  ...
  void* allocate(size_t bytes) {
    ...
    RG_LOG_DEBUG(std::format("pool_array: {:#x}", (long long)(pool_array)));
    //[free or not (pool)]
    ...
    RG_LOG_DEBUG(std::string("run allocate try, pool array count is ") + std::to_string(count_pools_from_main_pool));
    ...
    for (size_t i = 0; i < count_pools_from_main_pool && !valid_pool;i++) {
      ...
      if (now_pool != nullptr) {
        ...
        if (now_pool->magic == RG_MAG_NUM) [[likely]] {
          ...
          if (RG::K::Flags::has(now_pool->flags, MEM_FLAG::IS_POOL)) [[likely]] {
            //[if pool hasn't flag "read only" (very unlikely that it has have it)]
            if (!RG::K::Flags::has(now_pool->flags, MEM_FLAG::READ_ONLY)) [[likely]] {
              //[get pool cache]
              PoolCache* cache = (PoolCache*)(now_pool + 1);
              //[if pool used mem more then count for alloc plus other structure slop]
              RG_LOG_DEBUG(std::format("Pool id {:d}, mem id {:d}, size {:d}, used mem {:d}, try alloc {:d}, check of free {:d}", now_pool->pool_id, now_pool->id, now_pool->size, cache->used_mem, bytes, bytes + sizeof(PoolCache) + sizeof(MEM_HEAD) + sizeof(MEM_TAIL)));
              RG_LOG_DEBUG(std::format("size of mem head {:d}, size of mem tail {:d}, size of pool cache {:d}", sizeof(MEM_HEAD), sizeof(MEM_TAIL), sizeof(PoolCache)));
              //[if free size not enough]

              /*
              if (cache->near_free->size < (bytes + sizeof(MEM_HEAD) + sizeof(MEM_TAIL))) {
                RG_LOG_DEBUG("Tested cell debug: cache near free: {:#x}, size {:d}, m_h {:d}, m_t {:d}, next mem {:#x}", cache->near_free, cache->near_free->size, sizeof(MEM_HEAD), sizeof(MEM_TAIL), (((char*)cache->near_free) + cache->near_free->size + sizeof(MEM_HEAD) + sizeof(MEM_TAIL)));
                ...
                while (head_out->size < (bytes + sizeof(MEM_HEAD) + sizeof(MEM_TAIL))) {
                  if (RG::K::Flags::has(cache->near_free->flags, MEM_FLAG::INIT))
                    RG_LOG_DEBUG("Tested cell debug: now free: {:#x}, size {:d}, m_h {:d}, m_t {:d}, next mem {:#x}", head_out, head_out->size, sizeof(MEM_HEAD), sizeof(MEM_TAIL), (((char*)head_out) + head_out->size + sizeof(MEM_HEAD) + sizeof(MEM_TAIL)));
              ...
              if ((now_pool->size - cache->used_mem) < (bytes + sizeof(PoolCache) + sizeof(MEM_HEAD) * 2 + sizeof(MEM_TAIL) * 2)) {//todo there may be error (size mem_head * 2 and mem_tail * 2)
                ...
                if (RG::K::Flags::has(head_out->flags, MEM_FLAG::INIT)) {
                  while (head_out->size < (bytes /*+ sizeof(MEM_HEAD) + sizeof(MEM_TAIL)*/)) {//todo there may be a bug
                    RG_LOG_DEBUG(std::format("Tested cell debug: now free: {:#x}, size {:d}, m_h {:d}, m_t {:d}, next mem {:#x}", (long long)head_out, head_out->size, sizeof(MEM_HEAD), sizeof(MEM_TAIL), (long long)(((char*)head_out) + head_out->size + sizeof(MEM_HEAD) + sizeof(MEM_TAIL))));
                    head_out = (MEM_HEAD*)(((char*)head_out) + head_out->size + sizeof(MEM_HEAD) + sizeof(MEM_TAIL));
                    if (MEM_CELL_TEST(head_out)) {
                      RG_LOG_CRITICAL("Whath the logs");
                    }
                    if (!RG::K::Flags::has(head_out->flags, MEM_FLAG::INIT)) break;
                  }
                }
                if (RG::K::Flags::has(head_out->flags, MEM_FLAG::INIT)) {
                  RG::K::Flags::set(head_out->flags, MEM_FLAG::IS_USED);
                  return head_out + 1;
                }
                else {
                  RG_LOG_DEBUG(std::format("Check correct sys allocation: size = {:d}; mag is {:s}", now_pool->size, (now_pool->magic == RG_MAG_NUM) ? "correct" : "incorrect"));
                  //[fill mem vars]
                  head_out->size = bytes;
              ...
              RG_LOG_CRITICAL("pool is read only");
            }
          }
          else {
            RG_LOG_CRITICAL("pool is not pool lol (idk how do you do that...) late will be added fixex the pool (i think)");
          }
        }
        else {
          RG_LOG_CRITICAL("magic num is break... this is placeholder, late will be added fixes the pool (i think)");
        }
      }
        ...
        RG_LOG_DEBUG(std::string("Check correct sys allocation: size = ") + std::to_string(now_pool->size) + "; mag is " + ((now_pool->magic == RG_MAG_NUM) ? "correct" : "incorrect"));
        //[fill mem vars]
        head_out->size = bytes;
        head_out->magic = RG_MAG_NUM;
        head_out->flags = static_cast<uint32_t>(MEM_FLAG::IS_USED | MEM_FLAG::CUSTOM_POOL);
      ...
      RG_LOG_CRITICAL("Allocator::allocate(size_t): You've stumbled upon a case that's not ready yet.");
      return nullptr;
    }
  ...
  void deallocate(void* in) {
    if (MEM_CELL_TEST(in)) {
      RG_LOG_ERROR(std::format("deallocate: in ptr isn't RG type"));
      return;
    }
    MEM_HEAD* head = ((MEM_HEAD*)in) - 1;
    if (!RG::K::Flags::has(head->flags, MEM_FLAG::IS_USED)) {
      RG_LOG_WARN(std::format("deallocate: mem {:d} already dealocated", head->id));
      return;
    }
    if (!RG::K::Flags::has(head->flags, MEM_FLAG::CUSTOM_POOL)) {
      RG_LOG_WARN("deallocate: No optimization was applied because the MEM_FLAG::CUSTOM_POOL flag was disabled.");
    }
    else {
      MEM_HEAD** pool_array = (MEM_HEAD**)(pool.pool + 1);
    ...
    RG::K::Flags::clear(head->flags, MEM_FLAG::IS_USED);
    RG_LOG_MEM(std::format("Mem {:d} has been deallocated", head->id));
  }
  // void test() {
  //   RG_LOG_DEBUG("pool == " + std::to_string((long long)pool.pool));
  // }
  ~Allocator() {//* this is not safe
    // RG_LOG_DEBUG("Some allocator clearing...");
    if (pool.pool == nullptr) {
      RG_LOG_MEM("pool already free");
      return;
    }
    if (pool.pool->magic == RG_MAG_NUM) {
      ...
      RG_LOG_CRITICAL("How and for what??");
    }
    RG_LOG_DEBUG("Some allocator has clearing successfullyl!");

    RG::K::Flags::clear(pool.pool->flags, MEM_FLAG::LOCKED);
    pool.destruct();
  }
};
...
void Allocator_tests() {
  ...
  RG_LOG_INFO("Check size and mag num");
  RG_LOG_INFO(std::format("Size: {:d} mag num is {:s}", (((MEM_HEAD*)temp) - 1)->size, (((((MEM_HEAD*)temp) - 1)->magic == RG_MAG_NUM) ? "correct" : "incorrect")));
  RG_LOG_INFO("Try fill it");
  for (int i = 0;i < 20;i++) {
    temp[i] = (char)i;
  }
  // RG_LOG_INFO("Next");
  RG_LOG_INFO("Next");
  RG_LOG_INFO("Try init 40 bytes");
  char* temp2 = (char*)test.allocate(40);
  RG_LOG_INFO("Check size and mag num");
  RG_LOG_INFO(std::format("Size: {:d} mag num is {:s}", (((MEM_HEAD*)temp2) - 1)->size, (((((MEM_HEAD*)temp2) - 1)->magic == RG_MAG_NUM) ? "correct" : "incorrect")));
  RG_LOG_INFO("Try fill it");
  for (int i = 0;i < 40;i++) {
    temp2[i] = (char)i;
  }
  // RG_LOG_INFO
  // RG_LOG_INFO(std::string("Size: ") + std::to_string((((MEM_HEAD*)temp) - 1)->size) + ", mag num is " + (((((MEM_HEAD*)temp) - 1)->magic == RG_MAG_NUM) ? "correct" : "incorrect"));
  RG_LOG_INFO("Next");
  RG_LOG_INFO("Try deallocate 40 bytes");
  test.deallocate(temp2);
  RG_LOG_INFO("Next");
  RG_LOG_INFO("Try reinit 40 bytes");
  temp2 = (char*)test.allocate(40);
  RG_LOG_INFO("Check size and mag num");
  RG_LOG_INFO(std::format("Size: {:d} mag num is {:s}", (((MEM_HEAD*)temp2) - 1)->size, (((((MEM_HEAD*)temp2) - 1)->magic == RG_MAG_NUM) ? "correct" : "incorrect")));
  RG_LOG_INFO("Try read and init it");
  for (int i = 0;i < 40;i++) {
    rg_cout << (int)temp2[i] << ",";
    temp2[i] = (char)i;
  }
  RG_LOG_INFO("All test done");
};
