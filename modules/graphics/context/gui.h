...
void rgLoadGuiMatrix(LINK_GUI_TYPE type,float sizeWin = 1,bool dim = false)
{
	...
	tempOtn.x =(RG_MainWindow->set().resolution.x>RG_MainWindow->set().resolution.y)?((double)RG_MainWindow->set().resolution.x/RG_MainWindow->set().resolution.y):1;
	tempOtn.y = (double)RG_MainWindow->set().resolution.x/RG_MainWindow->set().resolution.y;
	switch(type)
		{
