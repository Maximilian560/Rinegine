...
{	
	...
	POINT2D<uint>pen;
	uint8 *texture = s_calloc<uint8>(size.x*size.y);//(uint8*)calloc(size.x*size.y,sizeof(uint8));

	



	if((characters = s_calloc<character>(countChar))==nullptr) throw(RG_ERROR_OUT_OF_MEMORY);
