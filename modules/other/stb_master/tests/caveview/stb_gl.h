...
int stbgl_TexImage2D_Extra(int texid, int w, int h, void *data, int chan, char *props, int preserve_data)
{
   ...
   if (data == nullptr && mipmaps == STBGL_GEN_MIPMAPS)
...
#ifdef STB_COMPRESS_DXT_BLOCK
   if (!is_compressed || !stbgl__CompressedTexImage2DARB || output_desc == STBGL_COMPRESSED_RGBA_S3TC_DXT3 || data == nullptr)
#endif
   {
      switch (mipmaps) {
         ...
         case STBGL_MIPMAPS: {
            ...
            if (data == nullptr) size = 0; // reuse same block of memory for all mipmaps
