...
void load_fonts(void)
{
   ...
   if (win == nullptr) win = getenv("SystemRoot");
...
void draw(void)
{
   draw_init();
   draw_world();
   stbwingraph_SwapBuffers(nullptr);
}
...
int winproc(void *data, stbwingraph_event *e)
{
   switch (e->type) {
      ...
      case STBWGE_char:
         switch(e->key) {
            case 27:
               stbwingraph_ShowCursor(nullptr,1);
...
void stbwingraph_main(void)
{
   ...
   stbwingraph_CreateWindow(1, winproc, nullptr, "tt", SIZE_X,SIZE_Y, 0, 1, 0, 0);
   stbwingraph_ShowCursor(nullptr, 0);
   load_fonts();
   initialized = 1;
