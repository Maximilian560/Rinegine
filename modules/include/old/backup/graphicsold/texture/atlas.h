...
public:
  ...
  void Font(
    #ifdef RG_WIN
    wstring path = L"/Windows/Fonts/segoeui.ttf", 
    #elif defined(RG_Linux)
    wstring path = L"/usr/share/fonts/cyr-sun16",
    #else
    wstring path,
    #endif
  ...
  void Font(
    ...
    if (FT_New_Face(library, path.c_str(), 0, &DefFace)) {
      RG_Debug::addl(RG_LOG_ERROR,"Font "+path+" not found");
    ...
    for (int i = 0; i < temp_enc.size(); i++) {
      ...
      for (int j = 0; j < temp_enc[i].size; j++) {
        ...
        if (pen.y >= size_mat.y) { RG_Debug::addl(RG_LOG_CRITICAL, "HOW!?"); }
    ...
    //RG_Debug::addl(RG_LOG_CRITICAL,"DEBUG");
    Font_Atl.init = true;
  ...
  void Gen() {
    ...
    if (names.size() == 0 || paths.size() == 0) {
      if (Font_Atl.size.x > 0) {
        ...
        return;
    }//RG_Debug::addl(RG_LOG_CRITICAL,"Error generating atlas, names or paths to textures were not found.");
    ...
    if (Font_Atl.init) {
      if (pen.x + Font_Atl.size.x > end_size.x) {
        if (pen.y + max.y >= end_size.y) { RG_Debug::addl(RG_LOG_CRITICAL, "Error gen atlas, size pen.y > end_size.y; end_size.y '" + to_string(end_size.y) + "' font.size '" + to_string(Font_Atl.size.y) + "'"); };
        pen.x = 0;
        pen.y += max.y + 1;
        max.y = 0;
      }
    ...
    for (RG_Texture::Standart& texture : Textures) {
      if (pen.x + texture.size.x > end_size.x) {
        if (pen.y + max.y >= end_size.y) { RG_Debug::addl(RG_LOG_CRITICAL, "Error gen atlas, size pen.y > end_size.y; end_size.y '" + to_string(end_size.y) + "' texture.size '" + to_string(texture.size.y) + "'"); };
        pen.x = 0;
        pen.y += max.y + 1;
        max.y = 0;
      }
  ...
  int operator[](string name) {
    ...
    RG_Debug::addl(RG_LOG_CRITICAL, "Atlas not found texture '" + name + "'");
    return -1;
  }
