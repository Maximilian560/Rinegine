...
namespace RG {
  ...
  struct Guis {
    ...
    Gui& operator[](rg_string in) const {
      ...
      RG_Debug::addl(RG_LOG_WARNING, in + RG_L" not found");
      return guis[0];
    }
    ...
    rg_string next(const rg_string& in)const {
      ...
      RG_Debug::addl(RG_LOG_ERROR, "Next in guis not found, exit");
      return RG_L"exit";
    }
    rg_string prev(const rg_string& in)const {
      ...
      RG_Debug::addl(RG_LOG_ERROR, "Prev in guis not found, exit");
      return RG_L"exit";
    }
  };
}         
...
int RG_Start(const RG::Guis& funcs, void(&RG_Prepare)() = RG_nop, RG_Window_Settings set = RG_MainSettingWindow) {
  ...
  RG_Debug::addl(RG_LOG_INFO, RG_L"Start GUIs program", false);

  /*while(NowPlay != -1){
    RG_Debug::addl(RG_LOG_INFO,"GUI id: "+to_string(NowPlay),false);
  ...
  while (NextPlay != RG_L"exit") {
    RG_Debug::addl(RG_LOG_INFO, RG_L"GUI code: " + NextPlay, false);
    if (!NextPlay.size())
    ...
    RG_Debug::addl(RG_LOG_INFO, RG_L"GUI id: " + NextPlay, false);
  ...
  RG_Debug::addl(RG_LOG_INFO, "Stop GUIs program", false);
  // window.close();
  // (&window)->~RG_Window();
  //}
  //RG_ERROR_LOG
