...
namespace RG {
  ...
  inline POINT3D<double> GlobalToGL(POINT3D<type>in) {
    POINT3D<double> out;
    out.x = (in.x - RG_Window_Standart->set().resolution.x / 2.) / RG_Window_Standart->set().resolution.x * 2.;
    out.y = -(in.y - RG_Window_Standart->set().resolution.y / 2.) / RG_Window_Standart->set().resolution.y * 2.;
    return out;
  }
  template<class type>
  ...
  namespace Graphic {
    namespace Flat {
      namespace Lock {
        ...
        public:
          void test() {
            RG_Debug::addl(RG_LOG_INFO, "Mesh test");
            RG_Debug::addl(RG_LOG_INFO, "Public:");
            RG_Debug::addl(RG_LOG_INFO, "GuiType: " + std::to_string((int)GuiType));
            RG_Debug::addl(RG_LOG_INFO, "is_gui: " + std::to_string(is_gui));
            RG_Debug::addl(RG_LOG_INFO, "is3d: " + std::to_string(is3d));
            RG_Debug::addl(RG_LOG_INFO, "hitbox: " + std::to_string(hitbox));
            RG_Debug::addl(RG_LOG_INFO, "collision: " + std::to_string(collision));
            RG_Debug::addl(RG_LOG_INFO, "animate: " + std::to_string(animate));
            RG_Debug::addl(RG_LOG_INFO, "custom_size: " + std::to_string(custom_size));
            RG_Debug::addl(RG_LOG_INFO, "textures: " + std::to_string(textures.size()));
            string temp;
            for (int i = 0; i < textures.size(); i++) {
              temp += std::to_string(textures[i]) + " ";
            }
            if (temp.size() > 0)RG_Debug::addl(RG_LOG_INFO, "Textures IDs: " + temp);

            RG_Debug::addl(RG_LOG_INFO, "Private:");
            RG_Debug::addl(RG_LOG_INFO, "pos: " + std::to_string(pos.x) + ", " + std::to_string(pos.y) + ", " + std::to_string(pos.z));
            RG_Debug::addl(RG_LOG_INFO, "rotate: " + std::to_string(rotate.x) + ", " + std::to_string(rotate.y) + ", " + std::to_string(rotate.z));
            RG_Debug::addl(RG_LOG_INFO, "size: " + std::to_string(size.x) + ", " + std::to_string(size.y));
            RG_Debug::addl(RG_LOG_INFO, "scale: " + std::to_string(scale.x) + ", " + std::to_string(scale.y));
            RG_Debug::addl(RG_LOG_INFO, "color: " + std::to_string(color.r) + ", " + std::to_string(color.g) + ", " + std::to_string(color.b) + ", " + std::to_string(color.a));
            RG_Debug::addl(RG_LOG_INFO, "Protected:");
            RG_Debug::addl(RG_LOG_INFO, "mat:\n" + mat.print_d());
            RG_Debug::addl(RG_LOG_INFO, "frame: " + std::to_string(frame));
            RG_Debug::addl(RG_LOG_INFO, "tempFrame: " + std::to_string(tempFrame));
            RG_Debug::addl(RG_LOG_INFO, "FrPeSe: " + std::to_string(FrPeSe));
            RG_Debug::addl(RG_LOG_INFO, "maxFrames: " + std::to_string(maxFrames));
          ...
          void MatInit() {
            ...
            if (textures.size() > 0 && !custom_size) {
              ...
              size.x = TextureSize.x / RG_Window_Virtual;
              size.y = TextureSize.y / RG_Window_Virtual;
              //RG_Debug::addl(RG_LOG_INFO, "Texture has, size = " + to_string(TextureSize.x) + ", " + to_string(TextureSize.y));
            }
          ...
          void TruePolygonInit() {
            if (collision) {
              ...
              RG_Debug::addl(RG_LOG_ERROR, "False true polygon init, collision is off");
            }
          }
          void TruePolygonUpdate() {
            if (collision) {
              ...
              RG_Debug::addl(RG_LOG_ERROR, "False polygon update, collision is off");
            }
            //rg_cout << endl;
          }
          /*void TruePolygonInit() {
          ...
          void draw(const RG_Camera& cam = *RG_Camera_StandartPtr) {
            ...
            } else RG_Debug::addl(RG_LOG_CRITICAL, "flat mesh gl vertex == -1");
            ...
            } else RG_Debug::addl(RG_LOG_CRITICAL, "flat mesh gl color == -1");//TODO fix everything
            //cam.loadMatrix();
          ...
          void draw_d(const RG_Camera& cam = *RG_Camera_StandartPtr) {
            RG_Debug::addl(RG_LOG_INFO, "draw start");
            RG_Debug::addl(RG_LOG_INFO, "Textures init");
            if (RG_Buffer::gl.textures.size() > 0) {
              RG_Debug::addl(RG_LOG_INFO, "Texture size > 0");
              if (animate)tempFrame += RG_Timer_StandartPtr->getBias() * FrPeSe;

              if (tempFrame > 1) {
                RG_Debug::addl(RG_LOG_INFO, "Next frame");
                frame += floor(tempFrame);
                tempFrame -= floor(tempFrame);
              }
              frame = frame % (maxFrames);//textures.size();
            ...
            RG_Debug::addl(RG_LOG_INFO, "Vertex init");
            ...
            RG_Debug::addl(RG_LOG_INFO, "Color init");
            if (RG_Buffer::gl.color.is_init()) {
              // glBindBuffer(GL_ARRAY_BUFFER, RG_Buffer::gl.color);
              RG_Buffer::gl.color.bind();
              rgColorPointer(4, GL_DOUBLE, 0, nullptr);
            ...
            RG_Debug::addl(RG_LOG_INFO, "Matrix init");
            if (is_gui) {
              RG_Debug::addl(RG_LOG_INFO, "GUI matrix");
              rgLoadGuiMatrix(GuiType);
            } else {
              //rgLoadCamMatrix();
              RG_Debug::addl(RG_LOG_INFO, "Cam matrix");
              cam.loadMatrix();
            }
            RG_Debug::addl(RG_LOG_INFO, "Object matrix init");
            rgLoadMatrixf(mat);


            RG_Debug::addl(RG_LOG_INFO, "Draw object");
            rgDrawElements(GL_TRIANGLES, RG_Buffer::raw.elements.size(), GL_UNSIGNED_INT, RG_Buffer::raw.elements.get_arr());
            RG_Debug::addl(RG_LOG_INFO, "Done");
          }
            ...
            if (is_gui) {
              ...
              if (RG_EXCEED_HITBOX_WARNING) {
                if (RG_Camera_StandartPtr->getRotate().x != 0 && RG_Camera_StandartPtr->getRotate().y != 0) {
                  RG_Debug::addl(RG_LOG_WARNING, "If you rotate the camera in x and y at the same time, the collision is broken, keep in mind that Action may not work correctly!");
          ...
          bool Collide(const Mesh& otherMesh, bool sticky = false, RG_Array<uint>ThisCollide = {}, RG_Array<uint> OtherCollide = {}) {
            ...
            if (ThisCollide.size() == 0) {
              for (uint i = 0; i < this->truePolygon.size(); ++i) {
                ...
                if (OtherCollide.size() == 0) {
                  ...
                  for (uint& j : OtherCollide) {
                    ...
                    if (j >= otherMesh.truePolygon.size()) { RG_Debug::addl(RG_LOG_ERROR, "Collide custom other collide index out of range, size == " + to_string(otherMesh.truePolygon.size()) + ", index = " + to_string(j)); continue; }
                    vec4<double> b1 = otherMesh.truePolygon[j];
                    vec4<double> b2 = otherMesh.truePolygon[(j + 1) % otherMesh.truePolygon.size()];
              ...
              for (uint& i : ThisCollide) {
                ...
                if (i >= this->truePolygon.size()) { RG_Debug::addl(RG_LOG_ERROR, "Collide custom this collide index out of range, size == " + to_string(this->truePolygon.size()) + ", index = " + to_string(i)); continue; }
                vec4<double> a1 = this->truePolygon[i];
                vec4<double> a2 = this->truePolygon[(i + 1) % this->truePolygon.size()];
                if (OtherCollide.size() == 0)
                  for (uint j = 0; j < otherMesh.truePolygon.size(); ++j) {
          ...
          /*
                    ...
                    //TruePolygonInit();
                      int intersections = 0;
                      vec2<double>point2 = {
                        (MPos.x - RG_Window_Standart->set().resolution.x / 2.) / RG_Window_Standart->set().resolution.x * 2.,
                        (MPos.y - RG_Window_Standart->set().resolution.y / 2.) / RG_Window_Standart->set().resolution.y * 2.
                      };
                      //vec2<double> hitbox = {};
                      //rg_cout << point << endl;
      ...
      public:
        ...
        int action(bool sticky = false) {
          ...
          if (is_gui) {
            ...
            if (GuiType == RG_GUI_LINK_LEFT_CENTER) { HPos.x = 0; HPos.y = RG_Window_Standart->set().resolution.y / 2.; }
            if (GuiType == RG_GUI_LINK_LEFT_BOTTOM) { HPos.x = 0; HPos.y = RG_Window_Standart->set().resolution.y; }

            if (GuiType == RG_GUI_LINK_CENTER_TOP) { HPos.x = RG_Window_Standart->set().resolution.x / 2.; HPos.y = 0; }
            if (GuiType == RG_GUI_LINK_CENTER_CENTER) { HPos.x = RG_Window_Standart->set().resolution.x / 2.; HPos.y = RG_Window_Standart->set().resolution.y / 2.; }
            if (GuiType == RG_GUI_LINK_CENTER_BOTTOM) { HPos.x = RG_Window_Standart->set().resolution.x / 2.; HPos.y = RG_Window_Standart->set().resolution.y; }

            if (GuiType == RG_GUI_LINK_RIGHT_TOP) { HPos.x = RG_Window_Standart->set().resolution.x; HPos.y = 0; }
            if (GuiType == RG_GUI_LINK_RIGHT_CENTER) { HPos.x = RG_Window_Standart->set().resolution.x; HPos.y = RG_Window_Standart->set().resolution.y / 2.; }
            if (GuiType == RG_GUI_LINK_RIGHT_BOTTOM) { HPos.x = RG_Window_Standart->set().resolution.x; HPos.y = RG_Window_Standart->set().resolution.y; }
          } else {
            HPos.x = RG_Window_Standart->set().resolution.x / 2.; HPos.y = RG_Window_Standart->set().resolution.y / 2.;
          }

          POINT2D <double>SIZE;

          SIZE.x = size.x * RG_Window_Standart->set().resolution.x * scale.x * ((float)RG_Window_Standart->set().resolution.y / RG_Window_Standart->set().resolution.x);
          SIZE.y = size.y * RG_Window_Standart->set().resolution.y * scale.y;
          //SIZE.z = size.z * scale.z;



          POINT2D<double> LeftTop = {
            (HPos.x + (pos.x / 2. * RG_Window_Standart->set().resolution.x) * ((float)RG_Window_Standart->set().resolution.y / RG_Window_Standart->set().resolution.x) - (SIZE.x / 2)),
            (HPos.y + (-pos.y / 2. * RG_Window_Standart->set().resolution.y)) - (SIZE.y / 2.)
          };
        ...
        void Set_Text() {
          ...
          if (text.size() > 0 && _text != text) {
            ...
            for (int i = 0; i < text.size(); i++) {
              if (text[i] == ' ') { space.x += RG_FONT_SIZE / RG_Window_Virtual / 2.; continue; }
              if (text[i] == '\n') { space.x = 0; space.y += RG_FONT_SIZE / RG_Window_Virtual; continue; }
              index++;
              const RG_Letter& letter = RG_Atlas_StandartPtr->Font_Get_Letter(text[i]);

              RG_Buffer::raw.vertex[index * 4 + 0] = {
                space.x,
                -space.y - (RG_FONT_SIZE - letter.bias.y) / RG_Window_Virtual,
                0 };

              RG_Buffer::raw.vertex[index * 4 + 1] = {
                space.x,
                -space.y - (RG_FONT_SIZE - letter.bias.y + letter.size.y) / RG_Window_Virtual,
                0 };

              RG_Buffer::raw.vertex[index * 4 + 2] = {
                (letter.size.x) / RG_Window_Virtual + space.x,
                -space.y - (RG_FONT_SIZE - letter.bias.y + letter.size.y) / RG_Window_Virtual,
                0 };
              ...
              //space.x += (letter.advance / 64. - letter.bias.x + letter.size.x) / RG_Window_Virtual;
              space.x += (letter.size.x + letter.bias.x + (RG_FONT_SIZE * _offset)) / RG_Window_Virtual;
    ...
    namespace Body {
        ...
        public:
          ...
          virtual void draw(RG_Camera& cam = *RG_Camera_StandartPtr) {
            ...
            if (RG_Buffer::gl.vertex.is_init()) {
              ...
              RG_Buffer::gl.vertex.bind();
              rgVertexPointer(3, GL_DOUBLE, 0, nullptr);
            } else RG_Debug::addl(RG_LOG_CRITICAL, "3d mesh vertex == -1");
            if (RG_Buffer::gl.color.is_init()) {
              // glBindBuffer(GL_ARRAY_BUFFER, RG_Buffer::gl.color);
              RG_Buffer::gl.color.bind();
              rgColorPointer(4, GL_DOUBLE, 0, nullptr);
            } else RG_Debug::addl(RG_LOG_CRITICAL, "3d mesh color == -1");//TODO fix everything
            //cam.loadMatrix();
      ...
      public:
        ...
        /* void Load(string path) {
           ...
           if (typefile == "obj") {
             ...
             RG_Debug::addl(RG_LOG_CRITICAL, "rgobj is not supported yet");
           } else {
             RG_Debug::addl(RG_LOG_CRITICAL, typefile + " is not supported, send me feedback if this type is worth adding https://t.me/T_Rinemest");
           }
         }*/
         /*void Load(string path) {
           string typefile;
             ...
             for (int i = 0; i < file.size();i++) {
               ...
               if (in > 32) {
                 ...
                 if (mode == MNone) {
                   ...
                   break;
                   default:
                   RG_Debug::addl(RG_LOG_ERROR, "Error mode by load model, mode error is " + to_string(mode));
                   }
                   temp.clear();
                   if (in == '\n') {
                     // update case
        ...
        void Create() {
          if (!RG_Buffer::raw.elements.size())RG_Debug::addl(RG_LOG_CRITICAL, "Body::Object not loaded");
          Mesh::MatInit();
          GL_Gen();
        }
        void draw(RG_Camera& cam = *RG_Camera_StandartPtr) {
          ...
          if (RG_Buffer::gl.color.is_init()) {
            ...
            RG_Buffer::gl.color.bind();
            rgColorPointer(4, GL_DOUBLE, 0, nullptr);
          } else RG_Debug::addl(RG_LOG_CRITICAL, "background color == -1");//TODO fix everything
          //cam.loadMatrix();
      ...
      public:
        ...
        void Create() {
          ...
          if (!Lock::Mesh::RG_Buffer::raw.vertex.size()) {
            RG_Debug::addl(RG_LOG_ERROR, "raw buffer vertex has empty");
          } if (!Lock::Mesh::RG_Buffer::raw.color.size()) {
            RG_Debug::addl(RG_LOG_ERROR, "raw buffer color has empty");
          } if (!Lock::Mesh::RG_Buffer::raw.elements.size()) {
            RG_Debug::addl(RG_LOG_ERROR, "raw buffer elements has empty");
          }
          MatInit();
          Lock::Mesh::RG_Buffer::GL_Gen();
          // if (textures.size() > 0) {
...
//         for(int i = 0; i < set.text.size();i++){
//           if(set.text[i] == ' '){ space.x += RG_FONT_SIZE / RG_Window_Virtual / 2.;continue; }
//           if(set.text[i] == '\n'){ space.x = 0;space.y += RG_FONT_SIZE / RG_Window_Virtual;continue; }
...
//           space.x += (letter.size.x) / RG_Window_Virtual;
//         }
//         buffers.GL_Gen();
//       }
//     }*/
...
//             space.x,
//             -space.y - (RG_FONT_SIZE - letter.bias.y) / RG_Window_Virtual, 0 };
//           buffers.raw.vertex[i * 4 + 1] = {
//             space.x,
//             -space.y - (RG_FONT_SIZE - letter.bias.y + letter.size.y) / RG_Window_Virtual, 0 };
...
//             if(RG_Atlas_StandartPtr->Font_Atl.buf.gl.textures.size() == 0)RG_Debug::addl(RG_LOG_CRITICAL, L"Что то тут не то...");
//             tempFrame += RG_Timer_StandartPtr->getBias() * FrPeSe;
...
//             rgVertexPointer(3, GL_DOUBLE, 0, nullptr);
//           }
//           else RG_Debug::addl(RG_LOG_CRITICAL, "text vertex == -1");
...
//             glBindBuffer(GL_ARRAY_BUFFER, buffers.gl.color);
//             rgColorPointer(4, GL_DOUBLE, 0, nullptr);
//           }else RG_Debug::addl(RG_LOG_CRITICAL, "text color == -1");
//           //cam.loadMatrix();
