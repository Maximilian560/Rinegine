...
public:
  ...
  RG_Block& get_block(POINT3D<int>pos) {
    //rg_cout<<pos.x<<" "<<pos.y<<" "<<pos.z<<endl;
  ...
  void init(uint seed) {
    ...
    RG_STANDART_COLOR.init(Chunks[0].buf.ebo().size(), RG_Array<COLOR4D<double>>(Chunks[0].buf.ebo().size(), { 1,1,1,1 }).get_arr(), GL_STATIC_DRAW);
