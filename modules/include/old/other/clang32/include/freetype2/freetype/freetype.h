...
#define ft_encoding_apple_roman     FT_ENCODING_APPLE_ROMAN
   ...
   *     `nullptr` (e.g., in fonts embedded in a PDF file).
   *
   ...
   *     this field is optional, and can be set to `nullptr`.  As for
   ...
   *     is set to `nullptr` if there is no bitmap strike.
   *
...
#define FT_HAS_SVG( face ) \
   ...
   *     FT_Face       face2;
   *     FT_Parameter  param = { FT_PARAM_TAG_IGNORE_SBIX, nullptr };
...
#define ft_open_params    FT_OPEN_PARAMS
   ...
   *     the font driver to use for opening the face.  If set to `nullptr`,
   ...
   *     equal to zero, it must be non-`nullptr`.
   *
   * @return:
   *   FreeType error code.  0~means success.
   *
   ...
   *     in `*aface` (if `aface` isn't `nullptr`); the only two useful fields in
...
#define FT_LOAD_TARGET_MODE( x )                               \
   ...
   * @input:
   *   matrix ::
   *     A pointer to the transformation's 2x2 matrix.  Use `nullptr` for the
   *     identity matrix.
   *   delta ::
   *     A pointer to the translation vector.  Use `nullptr` for the null
   *     vector.
   *
   * @note:
   ...
   * @output:
   *   matrix ::
   *     A pointer to a transformation's 2x2 matrix.  Set this to nullptr if you
   *     are not interested in the value.
   *
   *   delta ::
   *     A pointer to a translation vector.  Set this to nullptr if you are not
...
#define ft_kerning_unscaled  FT_KERNING_UNSCALED
   ...
   *     @random-seed).
   *
   *   Pass `nullptr` as `data` in @FT_Parameter for a given tag to reset the
   ...
   *
   * @return:
   *   A pointer to the face's PostScript name.  `nullptr` if unavailable.
   *
   * @note:
...
#define FT_FSTYPE_BITMAP_EMBEDDING_ONLY         0x0200
   ...
   *
   * @return:
   *   A pointer to an array of selector code points, or `nullptr` if there is
   ...
   *   active for the given character, or `nullptr` if the corresponding list is
   *   empty.
   *
   * @note:
   *   The last item in the array is~0; the array is owned by the @FT_Face
   ...
   *   (both default and non-default codes are returned) or `nullptr` if there
