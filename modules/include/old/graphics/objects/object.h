...
#define RG_HERE_FILE_NAME "graph/med/objects"
/*class RG_Object{
  POINT3D<double> pos;
  POINT3D<double> rot;
public:
    ...
    namespace Flat {
      namespace Lock {
        ...
        public:
          ...
          void Build() {
            if (maxFrames <= 0)RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "maxFrames<=0");
            if (frame >= maxFrames)RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "frame>=maxFrames");
          ...
          void MatInit() {
            ...
            if (textures.size() > 0 && !custom_size) {
              ...
              //RG_Debug::addl(RG_LOG_LOCK_INFO, "Texture has, size = " + to_string(TextureSize.x) + ", " + to_string(TextureSize.y));
            }
          ...
          void TruePolygonInit() {
            if (collision) {
              ...
              RG_Debug::addl(RG_LOG_LOCK_ERROR, "False true polygon init, collision is off");
            }
          }
          void TruePolygonUpdate() {
            if (collision) {
              ...
              RG_Debug::addl(RG_LOG_LOCK_ERROR, "False polygon update, collision is off");
            }
            //rg_cout << endl;
          }
          /*void TruePolygonInit() {
          ...
          void draw(const RG_Camera& cam = *RG_Camera_StandartPtr) {
            ...
            } else RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "flat mesh gl vertex == -1");
            ...
            } else RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "flat mesh gl color == -1");//TODO fix everything
            //cam.loadMatrix();
            ...
            if (is_gui) {
              ...
              if (RG_EXCEED_HITBOX_WARNING) {
                if (RG_Camera_StandartPtr->getRotate().x != 0 && RG_Camera_StandartPtr->getRotate().y != 0) {
                  RG_Debug::addl(RG_LOG_LOCK_WARNING, "If you rotate the camera in x and y at the same time, the collision is broken, keep in mind that Action may not work correctly!");
          ...
          bool Collide(const Mesh& otherMesh, bool sticky = false, RG_Array<uint>ThisCollide = {}, RG_Array<uint> OtherCollide = {}) {
            ...
            if (ThisCollide.size() == 0) {
              for (uint i = 0; i < this->truePolygon.size(); ++i) {
                ...
                if (OtherCollide.size() == 0) {
                  ...
                  for (uint& j : OtherCollide) {
                    ...
                    if (j >= otherMesh.truePolygon.size()) { RG_Debug::addl(RG_LOG_LOCK_ERROR, "Collide custom other collide index out of range, size == " + to_string(otherMesh.truePolygon.size()) + ", index = " + to_string(j)); continue; }
                    vec4<double> b1 = otherMesh.truePolygon[j];
                    vec4<double> b2 = otherMesh.truePolygon[(j + 1) % otherMesh.truePolygon.size()];
              ...
              for (uint& i : ThisCollide) {
                ...
                if (i >= this->truePolygon.size()) { RG_Debug::addl(RG_LOG_LOCK_ERROR, "Collide custom this collide index out of range, size == " + to_string(this->truePolygon.size()) + ", index = " + to_string(i)); continue; }
                vec4<double> a1 = this->truePolygon[i];
                vec4<double> a2 = this->truePolygon[(i + 1) % this->truePolygon.size()];
                if (OtherCollide.size() == 0)
                  for (uint j = 0; j < otherMesh.truePolygon.size(); ++j) {
      ...
      public:
        ...
        /*void draw(RG_Camera& cam = RG_Camera_StandartPtr) {
          ...
          if (RG_Buffer::gl.vertex != -1) {
            glBindBuffer(GL_ARRAY_BUFFER, RG_Buffer::gl.vertex);
            rgVertexPointer(3, GL_DOUBLE, 0, nullptr);
          } else RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "background vertex == -1");
          if (RG_Buffer::gl.color != -1) {
            glBindBuffer(GL_ARRAY_BUFFER, RG_Buffer::gl.color);
            rgColorPointer(4, GL_DOUBLE, 0, nullptr);
          } else RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "background color == -1");
          //cam.loadMatrix();
    ...
    namespace Body {
        ...
        public:
          ...
          virtual void draw(RG_Camera& cam = *RG_Camera_StandartPtr) {
            ...
            if (RG_Buffer::gl.vertex.is_init()) {
              ...
              RG_Buffer::gl.vertex.bind();
              rgVertexPointer(3, GL_DOUBLE, 0, nullptr);
            } else RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "3d mesh vertex == -1");
            if (RG_Buffer::gl.color.is_init()) {
              // glBindBuffer(GL_ARRAY_BUFFER, RG_Buffer::gl.color);
              RG_Buffer::gl.color.bind();
              rgColorPointer(4, GL_DOUBLE, 0, nullptr);
            } else RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "3d mesh color == -1");//TODO fix everything
            //cam.loadMatrix();
      ...
      public:
        ...
        /* void Load(string path) {
           ...
           if (typefile == "obj") {
             ...
             RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "rgobj is not supported yet");
           } else {
             RG_Debug::addl(RG_LOG_LOCK_CRITICAL, typefile + " is not supported, send me feedback if this type is worth adding https://t.me/T_Rinemest");
           }
         }*/
         /*void Load(string path) {
           string typefile;
             ...
             for (int i = 0; i < file.size();i++) {
               ...
               if (in > 32) {
                 ...
                 if (mode == MNone) {
                   ...
                   break;
                   default:
                   RG_Debug::addl(RG_LOG_LOCK_ERROR, "Error mode by load model, mode error is " + to_string(mode));
                   }
                   temp.clear();
                   if (in == '\n') {
                     // update case
        ...
        void Create() {
          if (!RG_Buffer::raw.elements.size())RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Body::Object not loaded");
          Mesh::MatInit();
          GL_Gen();
        }
        void draw(RG_Camera& cam = *RG_Camera_StandartPtr) {
          ...
          if (RG_Buffer::gl.color.is_init()) {
            ...
            RG_Buffer::gl.color.bind();
            rgColorPointer(4, GL_DOUBLE, 0, nullptr);
          } else RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "background color == -1");//TODO fix everything
          //cam.loadMatrix();
      ...
      public:
        ...
        void Create() {
          ...
          if (!Lock::Mesh::RG_Buffer::raw.vertex.size()) {
            RG_Debug::addl(RG_LOG_LOCK_ERROR, "raw buffer vertex has empty");
          } if (!Lock::Mesh::RG_Buffer::raw.color.size()) {
            RG_Debug::addl(RG_LOG_LOCK_ERROR, "raw buffer color has empty");
          } if (!Lock::Mesh::RG_Buffer::raw.elements.size()) {
            RG_Debug::addl(RG_LOG_LOCK_ERROR, "raw buffer elements has empty");
          }
          MatInit();
          Lock::Mesh::RG_Buffer::GL_Gen();
          // if (textures.size() > 0) {
...
//         //cout<<RG_Atlas_StandartPtr->get_textures().size()<<endl;
...
//             if(RG_Atlas_StandartPtr->Font_Atl.buf.gl.textures.size() == 0)RG_Debug::addl(RG_LOG_LOCK_CRITICAL, L"Что то тут не то...");
//             tempFrame += RG_Timer_StandartPtr->getBias() * FrPeSe;
...
//             rgVertexPointer(3, GL_DOUBLE, 0, nullptr);
//           }
//           else RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "text vertex == -1");
...
//             glBindBuffer(GL_ARRAY_BUFFER, buffers.gl.color);
//             rgColorPointer(4, GL_DOUBLE, 0, nullptr);
//           }else RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "text color == -1");
//           //cam.loadMatrix();
