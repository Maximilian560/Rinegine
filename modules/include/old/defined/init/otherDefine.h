...
#define RG_HERE_FILE_NAME "def/otherDef"
// Аргументы строки
RG_Array<string> RG_AMainArguments;
RG_Array<wstring> RG_WMainArguments;
#ifdef RG_UTF
RG_Array<wstring> &RG_MainArguments = RG_WMainArguments;
#else
RG_Array<string> &RG_MainArguments = RG_AMainArguments;
#endif
...
int RG_RunProgram(RG_ConfigRunProgram conf) {
	...
	if (conf.assinhrone) {
		...
		RG_LOG_LOCK_INFO("Create new process: '" + string(tempChar) + "' in " + (conf.otherCMD ? "other " : "same ") + "terminal");
...
int RG_RunProgram(RG_ConfigRunProgram conf) {
	RG_LOG_LOCK_WARNING("RG_RunProgram for Linux is not yet fully completed");
	return RG_CMD(conf.path);
}
#endif
...
string RG_AGetMainFolder() {
	//if(RG_AMainArguments.size()<1){RG_LOG_LOCK_ERROR("A non-critical error occurred during engine initialization. You probably started the engine initialization in your own main function, if you are not going to use the RG_MainFolder variable to get the path to the executable file, ignore this error, otherwise refer to the documentation or contact the developer");return "";}

	// if (RG_AMainArguments.size() < 1) {
	// 	RG_LOG_LOCK_ERROR("RG_AGetMainFolder error, RG_AMainArguments.size() < 1");
	// 	return "";
	// }
	// string fol;
	// int i;
	// for (i = RG_AMainArguments[0].size() - 1; i >= 0 && RG_AMainArguments[0][i] != '\\' && RG_AMainArguments[0][i] != '/'; i--);
	// for (int j = 0; j < i; j++) {
	// 	fol += RG_AMainArguments[0][j];
	// }
  ...
  if (getcwd(cwd, sizeof(cwd)) != NULL) {
    std::cout << "Текущая рабочая директория: " << cwd << std::endl;
		...
		RG_LOG_LOCK_ERROR("getcwd error");
		fol = "~/";
  }
	fol = cwd;
	return fol;
...
wstring RG_WGetMainFolder() {
	// if (RG_WMainArguments.size() < 1) {
	// 	RG_LOG_LOCK_ERROR("RG_WGetMainFolder error, RG_WMainArguments.size() < 1");
	// 	return L"";
	// }
	// wstring fol;
	// int i;
  ...
  if (getcwd(cwd, sizeof(cwd)) != NULL) {
	...
	fol = RG::utf8_decode(cwd);
  return fol;
}

#ifdef RG_UTF
inline wstring RG_GetMainFolder(){ return RG_WGetMainFolder(); }
#else 
inline string RG_GetMainFolder(){ return RG_AGetMainFolder(); }
#endif
	...
	struct vec4 {
		...
		void operator=(RG_Matrix<type2>in) {
			if (in.size(0) >= 4) {
				...
				RG_LOG_LOCK_CRITICAL("vec4 = matrix, size less 4 bun != 0, not support yet");
			} else {
				RG_LOG_LOCK_CRITICAL("vec4 = matrix, size == 0");
			}
		}
