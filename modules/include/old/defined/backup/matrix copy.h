...
public:
  ...
  type* operator[](const uint& i)const {
    if (i >= height)RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "matrix height is less than " + to_string(i + 1) + "(you set " + to_string(i) + ") and equal to " + to_string(height));
    if (!RG_EXCEED_LIMIN_MATRIX) { RG_Debug::addl(RG_LOG_LOCK_WARNING, "When using [y][x] it is possible to exceed the limit, check that x is not larger than the width of the matrix. This warning will no longer appear"); RG_EXCEED_LIMIN_MATRIX = 1; }
  ...
  type& get(const uint& x, const uint& y)const {
    if (y >= height) {
      RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Matrix height less than or equal to 'y' in 'get(x,y)'\n\
      'Height' = '" + to_string(height) + "', 'y' = '" + to_string(y) + "'");
    }
    if (x >= width) {
      RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Matrix width less than or equal to 'x' in 'get(x,y)'\n\
  ...
  type& getyx(to_rvalue(uint) y, to_rvalue(uint) x)const {
    if (y >= height) {
      RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Matrix height less than or equal to 'y' in 'getyx(y,x)'\n\
      'Height' = '" + to_string(height) + "', 'y' = '" + to_string(y) + "'");
    }
    if (x >= width) {
      RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Matrix width less than or equal to 'x' in 'getyx(y,x)'\n\
      'Width' = '" + to_string(width) + "', 'x' = '" + to_string(x) + "'");
    }
    return matrix[y * width + x];
  }
  type& get(to_rvalue(POINT2D<uint>)pos)const {
    if (pos.x >= width) {
      RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Matrix width less than or equal to 'pos.x' in 'get(POINT2D<uint>pos)'\n\
      'Width' = '" + to_string(width) + "', 'pos.x' = '" + to_string(pos.x) + "'");
    }
    if (pos.y >= height) {
      RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Matrix height less than or equal to 'pos.y' in 'get(POINT2D<uint>pos)'\n\
 ...
 // }
  ...
  /*
   void operator*=(const initializer_list<initializer_list<type>>& mat) {
     if (width != mat.begin()->size())RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Matrix multiplication error, the width of the first matrix does not match the height of the second matrix, width of the first '" + to_string(width) + "', height of the second matrix '" + to_string(mat.size()) + "'");
  ...
  void operator *=(to_rvalue(initializer_list<initializer_list<type2>>) mat) {
    if (width != mat.size())RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Matrix multiplication error, the width of the first matrix does not match the height of the second matrix, width of the first '" + to_string(width) + "', height of the second matrix '" + to_string(mat.size()) + "'");
    uint nsizex = mat.begin()->size();
    uint nsizey = height;
    //type* mt = (type*)calloc(nsizex*nsizey,sizeof(type));
    //type* mt = RG::s_new<type>(nsizex*nsizey);
  ...
  void operator *=(const RG_Matrix<type>& mat) {
    if (width != mat.size(1))RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Matrix multiplication error, the width of the first matrix does not match the height of the second matrix, width of the first '" + to_string(width) + "', height of the second matrix '" + to_string(mat.size(1)) + "'");
  ...
  RG_Matrix<type>operator%(to_rvalue(RG_Matrix<type2>)in)const {
    if (width != in.width || height != in.height)RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Matrix % error, matrix sizes do not match");
  ...
  RG_Matrix<type>operator*(const RG_Array<type>& mat)const {
    ...
    //if(width!=1)RG_Debug::addl(RG_LOG_LOCK_CRITICAL,"Matrix multiplication error, the width of the first matrix does not match the height of the second matrix, width of the first '"+to_string(width)+"', height of the second matrix '"+to_string(mat.size(1))+"'");
    if (width != 1)RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Matrix multiplication error (matrix by array) width of the matrix does not match array height (1), width of the matrix '" + to_string(width) + "', array height '1'");
  ...
  void operator+=(const RG_Matrix<type>& mat) {
    if (width != mat.width || height != mat.height)RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Matrix += error, matrix sizes do not match");
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        get(j, i) += mat.get(j, i);
      }
  ...
  void operator-=(const RG_Matrix<type>& mat) {
    //wcout<<"\nTEST\n";
    if (width != mat.width || height != mat.height)RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Matrix -= error, matrix sizes do not match, this mat width/height = '" + to_string(width) + "'/'" + to_string(height) + "', right mat width/height = '" + to_string(mat.size(0)) + "'/'" + to_string(mat.size(1)) + "'");
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        get(j, i) -= mat.get(j, i);
      }
