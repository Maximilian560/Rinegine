...
namespace RG {
  ...
  public:
    ...
    operator type& () const {
      if (!is_init()) {
        RG_Debug::addl(RG_LOG_LOCK_ERROR, "Dereferencing null pointer");
      }
      return *_pointer;
    }
  ...
  public:
    ...
    void operator=(const Pointer& in){
      ...
      _pointer = in._pointer;
      _init = in._init;
      RG_Debug::addl(RG_LOG_LOCK_WARNING,"pointer.h:"+to_string(__LINE__)+" Pointer copy constructor");
    }
    //CLEAR
    void clear() {
...
//     if(SIZE==0)RG_Debug::addl(RG_LOG_LOCK_CRITICAL,"Size array = 0");
//     if(i>=0){
//       if(i<=SIZE){
//         return array[i];
//       }
//       else {
//         RG_Debug::addl(RG_LOG_LOCK_CRITICAL,"i > 0. Size array loss then 'i' in operator[](uint i), i = '"+to_string(i)+"', size = '"+to_string(SIZE)+"'");
//       }
//     }
//     else{
//       if(int(SIZE)+i<SIZE){
...
//       }
//       else{
//         RG_Debug::addl(RG_LOG_LOCK_CRITICAL,"i < 0. i + array size is greater than the array size, array size = '"+to_string(SIZE)+"', i = '"+to_string(i)+"', i + array size'"+to_string(int(SIZE)+i)+"'");
//       }
//     }
//     RG_Debug::addl(RG_LOG_LOCK_CRITICAL,"Array unknow error");
//     return array[0];
//   }
...
//     INIT = in.INIT;
//     noclear = in.noclear;
//     RG_Debug::addl(RG_LOG_LOCK_WARNING,"A copy of the array has been created, which points to the same array that is associated with the original, thus, when the original is deleted, crashes due to access to unallocated memory or garbage return are possible.");
//   }*/
...
//   }
//   void operator-=(to_rvalue(RG_Pointer<type>)in){
//     if(SIZE!=in.size())RG_Debug::addl(RG_LOG_LOCK_CRITICAL,"Array subtraction error arrays size does not match, first array size '"+to_string(SIZE)+"', second array size '"+to_string(in.size())+"'");
//     for(int i =0; i<SIZE;i++){
//       array[i] -= in[i];
//     }
//   }
//   void operator+=(to_rvalue(RG_Pointer<type>)in){
//     if(SIZE!=in.size())RG_Debug::addl(RG_LOG_LOCK_CRITICAL,"Array addition error arrays size does not match, first array size '"+to_string(SIZE)+"', second array size '"+to_string(in.size())+"'");
//     for(int i =0; i<SIZE;i++){
//       array[i] += in[i];
//     }
//   }
...
//     if(SIZE!=mat.size(1))RG_Debug::addl(RG_LOG_LOCK_CRITICAL,"Array multiplication error (array by matrix) array size does not match matrix width, array size '"+to_string(SIZE)+"', matrix height '"+to_string(mat.size(1))+"'");
...
//   /*RG_Pointer<type>operator%(to_rvalue(RG_Pointer<type>)in)const{
//     if(SIZE!=in.size())RG_Debug::addl(RG_LOG_LOCK_CRITICAL,"Array multiplication error arrays size does not match, first array size '"+to_string(SIZE)+"', second array size '"+to_string(in.size())+"'");
