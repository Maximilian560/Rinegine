...
namespace RG {
	...
	inline string FileLoad(string path) {
		//uint temp = 0;
	...
	public:
		...
		WIN32_FIND_DATA* init(const rg_string& path) {
			if (!_init) {
				...
				RG_Debug::addl(RG_LOG_ERROR, "FileFinder is already initialized");
		...
		WIN32_FIND_DATA* next() {
			if (!_init) {
				RG_Debug::addl(RG_LOG_ERROR, "FileFinder is not initialized");
				return nullptr;
			}
