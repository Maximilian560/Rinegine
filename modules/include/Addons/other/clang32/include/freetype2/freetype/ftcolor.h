...
#define FT_PALETTE_FOR_DARK_BACKGROUND   0x02
   ...
   *     0xFFFF.
   *
   *     `nullptr` if the font's 'CPAL' table doesn't contain appropriate data.
   *
   *   palette_flags ::
   ...
   *     `FT_Palette_Data` structure).  If `apalette` is set to `nullptr`, no
   *     array gets returned (and no color entries can be modified).
   *
   *     In case the font doesn't support color palettes, `nullptr` is returned.
   *
   * @return:
   *   FreeType error code.  0~means success.
   *
   ...
   *     to `nullptr` before the first call of @FT_Get_Color_Glyph_Layer.
   */
   ...
   *     `iterator->p` to `nullptr`.  For all following calls, simply use the
   *     same object again.
   *
   * @output:
   *   aglyph_index ::
   ...
   * @fields:
   *   p ::
   *     An internal offset to a Paint table, needs to be set to nullptr before
