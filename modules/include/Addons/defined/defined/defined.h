...
#define RG_HERE_FILE_NAME "def/def"
#include <cmath> // Подключение математической библиотеки
#include <string> // Подключение библиотеки для работы со строками
#include <format> // Подключение библиотеки для работы со строками
...
#define rg_cout cout
#define rg_ostrem ostream

inline std::ostream& operator<<(std::ostream& out, const std::string& text) {
	return out.write(text.data(), text.size());
}
...
#define rg_count rg_cout<<(rg_count_temp++)<<", "<<__FILE__<<":"<<__LINE__<<endl
...
namespace Rinegine::Kernel {
	namespace Lock {
		...
		void addl(Rinegine::Kernel::Log::Types type = Rinegine::Kernel::Log::DEBUG, string text = "empty", bool print = true, string file = __FILE__, int line = __LINE__) {
			...
			TempError += (char(line >> 8 * 1));
			TempError += (char(line >> 8 * 2));
			TempError += (char(line >> 8 * 3));
			TempError += text;
			TempError += char(3);
			TempError += file;
			TempError += char(4);
	...
	namespace Lock {
		void addl(Rinegine::Kernel::Log::Types type = Rinegine::Kernel::Log::DEBUG, wstring text = L"empty", bool print = true, wstring file = utf8_decode(__FILE__), int line = __LINE__) {
			...
			TempError += (char(line >> 8 * 2));
			TempError += (char(line >> 8 * 3));
			TempError += utf8_encode(text);
			TempError += char(3);
			TempError += utf8_encode(file);
			TempError += char(4);
			_logs++;
		}
	}
...
inline int RG_TestKeyIs(int in, bool sticky = 1) {
	int out = RG_KEYS_TEST[in];
	if (sticky){RG_KEYS_TEST[in]--;}
	return out;
}
...
public:
	...
	static string HourA() {
		string temp = std::to_string(SystemTime.wHour);
		return string((temp.length() == 1 ? "0" : "") + temp);
	}
...
void RG_SetColorConsol(int col){
	rg_cout<<"\x1b["+to_string(col)+"m";
}
