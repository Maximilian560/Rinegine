...
public:
  ...
  type find(string ii) {
    ...
    RG_Debug::addl(RG_LOG_CRITICAL, "RG_Map error, '" + ii + "' not found");
    return map[0].value;

  }
  type find(int ii) {
    if (ii < 0)RG_Debug::addl(RG_LOG_CRITICAL, "RG_Map error, '" + to_string(ii) + "' less than zero");
    if (ii > map.size())RG_Debug::addl(RG_LOG_CRITICAL, "RG_Map error, '" + to_string(ii) + "' is greater than RG_Map size");
    return map[ii].value;
...
public:
  ...
  type& find(wstring ii) {
    ...
    RG_Debug::addl(RG_LOG_CRITICAL, L"RG_WMap find wstring error, '" + ii + L"' not found");
    return map[0].value;

  }
  type& find(int ii) {
    if (ii < 0)RG_Debug::addl(RG_LOG_CRITICAL, "RG_WMap find int error, '" + to_string(ii) + "' less than zero");
    if (ii > map.size())RG_Debug::addl(RG_LOG_CRITICAL, "RG_WMap find int error, '" + to_string(ii) + "' is greater than RG_WMap size");
    return map[ii].value;
  }
  ...
  RG_MapData<wstring, type>& findchar(wchar_t in) {
    ...
    RG_Debug::addl(RG_LOG_CRITICAL, L"RG_WMap find wchar error, '" + ii + L"' not found");
    return map[0];
  }
  /*type operator[](int i){
    return find(i);
