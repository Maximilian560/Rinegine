...
public:
  ...
  void Font(wstring path = L"\\Windows\\Fonts\\segoeui.ttf", int sizefont = 50, RG_Array<wstring> lang = {}) {
  ...
  void Font(string path = "\\Windows\\Fonts\\segoeui.ttf", int sizefont = 50, RG_Array<string>lang = {}) {
    ...
    //wcout << to_wstring(count_let) << endl;
    //wcout << to_wstring(size_mat.x) << L" " << to_wstring(size_mat.y) << L" " << to_wstring(size_mat.x * (size_mat.y - 1)) << endl;
    POINT2D<int> pen;
    int ti = 0;
    ...
    for (int i = 0; i < temp_enc.size();i++) {
      ...
      for (int j = 0; j < temp_enc[i].size;j++) {
        ...
        if (pen.y >= size_mat.y) { RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "HOW!?"); }
        ...
        //cout<<letter.size.x<<" "<<DefFace->glyph->bitmap.width<<endl;
        ...
        //wcout<<to_wstring(pen.x)<<"|"<<to_wstring(pen.y)<<L" ";
        pen.x++;
        ti++;
      }
    }
    ...
    //wcout<<endl;
        //wcout<<wchar_t(temp_enc[i].pos+j  )<<L" = "<<to_wstring(temp_enc[i].pos+j)<<endl;
    //stbi_write_png("atlas_font.png",Font_Atl.font.size.x,Font_Atl.font.size.y,4,Font_Atl.font.raw.get_mat(),0);
    //RG_Debug::addl(RG_LOG_LOCK_CRITICAL,"DEBUG");
    Font_Atl.init = true;
  ...
  /*||||||||* /
            ...
            /*void Font_Buf_Gen(){
              ...
              for(RG_Letter& letter : Font_Atl.letters){
                ...
                Font_Atl.buf.raw.vertex[i * 4 + 0] = {
                  0,
                  - (RG_FONT_SIZE - letter.bias.y) / RG_Window_Virtual, 0};
                Font_Atl.buf.raw.vertex[i * 4 + 1] = {
                  0,
                  - (RG_FONT_SIZE - letter.bias.y + letter.size.y) / RG_Window_Virtual, 0};
                Font_Atl.buf.raw.vertex[i * 4 + 2] = {
                  (letter.size.x) / RG_Window_Virtual,
                  - (RG_FONT_SIZE - letter.bias.y + letter.size.y) / RG_Window_Virtual, 0};
                Font_Atl.buf.raw.vertex[i * 4 + 3] = {
                  (letter.size.x) / RG_Window_Virtual + 0,
                  - (RG_FONT_SIZE - letter.bias.y) / RG_Window_Virtual, 0};
  ...
  void Gen() {
    ...
    //if(names.size()==0||paths.size()==0)RG_Debug::addl(RG_LOG_LOCK_CRITICAL,"Error generating atlas, names or paths to textures were not found.");
    if (names.size() != paths.size())RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Atlas generation error, number of textures and their paths do not match.");
    if (names.size() == 0 || paths.size() == 0) {
      if (Font_Atl.size.x > 0) {
        raw = Font_Atl.font.raw;
      } else
        return;
    }//RG_Debug::addl(RG_LOG_LOCK_CRITICAL,"Error generating atlas, names or paths to textures were not found.");
    ...
    if (Font_Atl.init) {
      if (pen.x + Font_Atl.size.x > end_size.x) {
        if (pen.y + max.y >= end_size.y) { RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Error gen atlas, size pen.y > end_size.y; end_size.y '" + to_string(end_size.y) + "' font.size '" + to_string(Font_Atl.size.y) + "'"); };
        pen.x = 0;
        pen.y += max.y + 1;
        max.y = 0;
      }
    ...
    for (RG_Texture::Standart& texture : Textures) {
      if (pen.x + texture.size.x > end_size.x) {
        if (pen.y + max.y >= end_size.y) { RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Error gen atlas, size pen.y > end_size.y; end_size.y '" + to_string(end_size.y) + "' texture.size '" + to_string(texture.size.y) + "'"); };
        pen.x = 0;
        pen.y += max.y + 1;
        max.y = 0;
      }
    ...
    //GetLastError();
    //RG_Debug::addl(RG_LOG_LOCK_INFO,"Atlas successfully generated");
    //RG_Debug::stop();
    _INIT = true;
  ...
  int operator[](string name) {
    ...
    RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Atlas not found texture '" + name + "'");
    return -1;
  }
...
public:
  static void Texture_On() {
    if (textures) {
      //RG_Debug::addl(RG_LOG_LOCK_INFO,"Atlas already on");
      return;
    };
    if (RG_Atlas_StandartPtr->get_id() == -1) {
      RG_Debug::addl(RG_LOG_LOCK_CRITICAL, "Atlas not prepare");
    }
    textures = 1;
  ...
  static void Texture_Off() {
    if (!textures) {
      //RG_Debug::addl(RG_LOG_LOCK_INFO,"Atlas already off");
      return;
    }
    textures = 0;
