...
void RG_PREPARE_SHADER(to_rvalue(uint) prog){
  ...
  RG_TEXTURE_COORD_ARRAY_POS = glGetAttribLocation(prog,"rg_TextureCoord");
	...
	//RG_Debug::addl(RG_LOG_LOCK_INFO,"rg_Vertex:" + to_string(RG_VERTEX_ARRAY_POS)+", rg_Color:" + to_string(RG_COLOR_ARRAY_POS) + ", rg_TextureCoord:" + to_string(RG_TEXTURE_COORD_ARRAY_POS) + ", rg_Offset:" + to_string(RG_OFFSET_ARRAY_POS));
