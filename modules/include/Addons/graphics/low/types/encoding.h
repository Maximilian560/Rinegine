...
#define RG_HERE_FILE_NAME "graph/low/types/encoding(old)"
...
public:
  ...
  RG_List(initializer_list<type>t) {
...
class RG_ENCODE {
  ...
  inline static void init() {
    if(_init)return;
    _init = 1;
    encode = RG_Array<RG_ENCODE_TYPE>{
  ...
  //https://symbl.cc/en/unicode/blocks/specials/ //TODO NEED COMPLITED
...
public:
  ...
  static RG_ENCODE_VALUE get(string nam) {
    ...
    int id = get_id(nam);
    if (id == -1)RG_LOG_LOCK_CRITICAL("encoding '" + nam + "' not found!");
    size = encode[id].size;
    //rg_cout<<endl<<L"name = "<<RG::utf8_decode(nam)<<endl;
    //rg_cout<<L"id = "<<to_wstring(id)<<endl;
    //rg_cout<<L"size = "<<to_wstring(size)<<endl;
    for (int i = 0; i < id; i++) {
      pos += encode[i].size;
    }
    //rg_cout<<L"pos = "<<to_wstring(pos)<<endl<<endl;
    return { pos,size };
  }
