...
int stbwingraph_ChangeResolution(unsigned int w, unsigned int h, unsigned int bits, int use_message_box)
{
   ...
   int i, tries=0;
   for (i=0; ; ++i) {
      int success = EnumDisplaySettings(nullptr, i, &mode);
      if (!success) break;
   ...
   if (!tries) {
      if (use_message_box)
         stbwingraph_MessageBox(stbwingraph_primary_window, MB_ICONERROR, nullptr, "The resolution %d x %d x %d-bits is not supported.", w, h, bits);
      return FALSE;
   }
   ...
   switch (res) {
      ...
      case DISP_CHANGE_RESTART:
         if (use_message_box)
            stbwingraph_MessageBox(stbwingraph_primary_window, MB_ICONERROR, nullptr, "Please set your desktop to %d-bit color and then try again.");
         return FALSE;

      case DISP_CHANGE_FAILED:
         if (use_message_box)
            stbwingraph_MessageBox(stbwingraph_primary_window, MB_ICONERROR, nullptr, "The hardware failed to change modes.");
         return FALSE;
      ...
      default:
         if (use_message_box)
            stbwingraph_MessageBox(stbwingraph_primary_window, MB_ICONERROR, nullptr, "An unknown error prevented a change to a %d x %d x %d-bit display.", w, h, bits);
         return FALSE;
   }
}
           ...
           int width, int height, int fullscreen, int resizeable, int dest_alpha, int stencil)
{
   ...
   win = CreateWindow("zwingraph", text ? text : "sample", dwstyle,
                      ...
                      nullptr, nullptr, stbwingraph_app, z);
...
void *stbwingraph_CreateWindowSimple(stbwingraph_window_proc func, int width, int height)
{
   ...
   #endif
   return stbwingraph_CreateWindow(1, func, nullptr, nullptr, width, height, fullscreen, 1, 0, 0);
}
...
void *stbwingraph_CreateWindowSimpleFull(stbwingraph_window_proc func, int fullscreen, int ww, int wh, int fw, int fh)
{
   ...
   return stbwingraph_CreateWindow(1, func, nullptr, nullptr, ww, wh, fullscreen, 1, 0, 0);
}
...
void stbwingraph_DestroyWindow(void *window)
{
   ...
   if (stbwingraph_primary_window == window)
      stbwingraph_primary_window = nullptr;
}
...
int stbwingraph_MainLoop(stbwingraph_update func, float mintime)
{
   ...
   for(;;) {
      ...
      if (!is_animating || PeekMessage(&msg, nullptr, 0, 0, PM_NOREMOVE)) {
...
void stbwingraph_SwapBuffers(void *win)
{
   ...
   if (win == nullptr) win = stbwingraph_primary_window;
