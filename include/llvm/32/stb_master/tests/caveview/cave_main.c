...
static void print_string(float x, float y, char *text, float r, float g, float b)
{
   ...
   num_quads = stb_easy_font_print(x, y, text, nullptr, buffer, sizeof(buffer));
...
void error(char *s)
{
   SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Error", s, nullptr);
   exit(0);
}
...
int SDL_main(int argc, char **argv)
{
   ...
   if (glDebugMessageCallbackARB) {
      glDebugMessageCallbackARB(gl_debug, nullptr);
