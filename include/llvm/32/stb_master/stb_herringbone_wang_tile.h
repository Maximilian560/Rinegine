...
int main(int argc, char **argv)
{
   ...
   data = stbi_load(argv[1], &w, &h, nullptr, 3);
   xs = atoi(argv[2]);
   ys = atoi(argv[3]);
   if (data == nullptr) {
   ...
   srand(time(nullptr));
   stbhw_generate_image(&ts, nullptr, data, xs*3, xs, ys);
...
// weighting should be nullptr, as non-nullptr weighting is currently untested
...
STBHW_EXTERN void stbhw_get_template_size(stbhw_config *c, int *w, int *h)
{
   stbhw__get_template_info(c, w, h, nullptr, nullptr);
}
...
static int stbhw__process_template(stbhw__process *p)
{
   ...
   stbhw__get_template_info(c, &size_x, &size_y, nullptr, nullptr);
   ...
   if (ts->is_corner) {
      ...
      for (j=0; j < ymax; ++j) {
         for (i=0; i < xmax; ++i) {
            ...
            if (weighting==nullptr || weighting[p]==0 || cc[p] == 1)
      ...
      for (j=0; j < ymax-3; ++j) {
         for (i=0; i < xmax-3; ++i) {
            ...
            if (stbhw__match(i,j) && stbhw__match(i,j+1) && stbhw__match(i,j+2)
               ...
               int p = ((i+1)-(j+1)+1) & 3;
               if (cc[p] > 1)
                  c_color[j+1][i+1] = stbhw__change_color(c_color[j+1][i+1], cc[p], weighting ? weighting[p] : nullptr);
            }
            if (stbhw__match(i,j) && stbhw__match(i+1,j) && stbhw__match(i+2,j)
                && stbhw__match(i,j+1) && stbhw__match(i+1,j+1) && stbhw__match(i+2,j+1)) {
               int p = ((i+2)-(j+1)+1) & 3;
               if (cc[p] > 1)
                  c_color[j+1][i+2] = stbhw__change_color(c_color[j+1][i+2], cc[p], weighting ? weighting[p] : nullptr);
            }
         }
      }
      #endif
...
STBHW_EXTERN int stbhw_build_tileset_from_image(stbhw_tileset *ts, unsigned char *data, int stride, int w, int h)
{
   ...
   stbhw__get_template_info(&c, nullptr, nullptr, &h_count, &v_count);
