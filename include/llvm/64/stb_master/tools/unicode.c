...
table pack_for_mode(table *t, int mode, char *table_name)
{
   ...
   uval *packed = nullptr;
   uval *index = nullptr;
   uval *indirect = nullptr;
   uval *specials = nullptr;
   ...
   if (table_name) {
      if (t->splittable)
         ...
         output_table(table_name, "", packed, stb_arr_len(packed), t->has_sign, nullptr);
   }
   ...
   if (table_name)
      output_table(table_name, "_default", specials, stb_arr_len(specials), 0, nullptr);
   ...
   if (table_name && stb_arr_len(indirect)) {
      if (mi.trim_end)
         ...
         output_table(table_name, "_index", indirect, stb_arr_len(indirect), 0, nullptr);
      }
...
void optimize_table(table *t, char *table_name)
{
   ...
   if (s.splittable)
      ...
      output_table(table_name, "_1", s.input, s.length, 0, nullptr);
}
