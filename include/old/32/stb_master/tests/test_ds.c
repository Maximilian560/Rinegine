...
{
  struct { int key,value; } *map=nullptr;
  int i,j,n,k;
  for (i=0; i < a; ++i)
    hmput(map,i,i+1);
  for (n=0; n < count; ++n) {
...
{
  struct { unsigned int key,value; } *map=nullptr;
...
{
  struct { str32 key; int value; } *map=nullptr;
  int i,j,n;
  str32 key = { 0 };
  for (i=0; i < a; ++i) {
    key.n[0] = i;
...
{
  struct { str256 key; int value; } *map=nullptr;
  int i,j,n;
  str256 key = { 0 };
  for (i=0; i < a; ++i) {
    key.n[0] = i;
...
{
  struct { size_t key,value; } *map=nullptr;
  int i,j,n,k;
  for (i=0; i < a; ++i)
    hmput(map,i,i+1);
  if (!include_startup)
