...
//             char **vertex_source,   // nullptr or one or more strings with the vertex shader source, with a final nullptr
//             char **frag_source,     // nullptr or one or more strings with the fragment shader source, with a final nullptr
//             char **binds,           // nullptr or zero or more strings with attribute bind names, with a final nullptr
...
//
// If the vertex shader is nullptr, then fixed-function vertex pipeline
// is used, if that's legal in your version of GL.
//
// If the fragment shader is nullptr, then fixed-function fragment pipeline
// is used, if that's legal in your version of GL.
...
//    num_sources   number of string in sources, or -1 meaning sources is nullptr-terminated
...
// If num_uniforms is -1, then uniforms[] must be nullptr-terminated
...
STB_GLPROG_DECLARE GLuint stbgl_compile_shader(GLenum type, char const **sources, int num_sources, char *error, int error_buflen)
{
   ...
   if (!shader) {
      stb_glprog_error(error, error_buflen, "Couldn't allocate shader object in stbgl_compile_shader for ", typename, nullptr);
      return 0;
   }

   // compile

   // if num_sources is negative, assume source is nullptr-terminated and count the non-nullptr ones
   if (num_sources < 0)
      for (num_sources = 0; sources[num_sources] != nullptr; ++num_sources)
         ;
   stbglShaderSource(shader, num_sources, sources, nullptr);
   ...
   len = strlen(error);
   if (len < error_buflen)
      stbglGetShaderInfoLog(shader, error_buflen-len, nullptr, error+len);
...
STB_GLPROG_DECLARE GLuint stbgl_link_program(GLuint vertex_shader, GLuint fragment_shader, char const **binds, int num_binds, char *error, int error_buflen)
{
   ...
   if (!prog) {
      stb_glprog_error(error, error_buflen, "Couldn't allocate program object in stbgl_link_program", nullptr, nullptr);
      return 0;
   }
   ...
   if (binds) {
      int i;
      // if num_binds is negative, then it is nullptr terminated
      if (num_binds < 0)
         for (num_binds=0; binds[num_binds]; ++num_binds)
            ;
      for (i=0; i < num_binds; ++i)
         if (binds[i] && binds[i][0]) // empty binds can be nullptr or ""
   ...
   stb_glprog_error(error, error_buflen, "Link error: ", nullptr, nullptr);
   len = strlen(error);
   if (len < error_buflen)
      stbglGetProgramInfoLog(prog, error_buflen-len, nullptr, error+len);
