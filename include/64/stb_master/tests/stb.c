...
void test_threads(void)
{
   ...
   stest = stb_sem_new(8);
   mutex = stb_mutex_new();
   stb_work(thread_3, nullptr, nullptr);
   stb_work(thread_3, nullptr, nullptr);
   stb_work(thread_3, nullptr, nullptr);
   stb_work(thread_3, nullptr, nullptr);
   stb_work(thread_3, nullptr, nullptr);
...
void test_threads2(void)
{
   ...
   stb_work_reach(thread4, nullptr, nullptr, synch);
...
void bst_test(void)
{
   ...
   for (z=0; z < 10; ++z) {
      ...
      for (i=0; i < 500; ++i)  {
         ...
         #endif
         for (j=0; j <= i; ++j)
            c(btest_find(root, items[j]) != nullptr, "stb_bst 1");
         for (   ; j < 500; ++j)
            c(btest_find(root, items[j]) == nullptr, "stb_bst 2");
      }

      t = btest_first(root);
      for (i=0; i < 500; ++i)
         t = btest_next(root,t);
      c(t == nullptr, "stb_bst 5");
      t = btest_last(root);
      for (i=0; i < 500; ++i)
         t = btest_prev(root,t);
      c(t == nullptr, "stb_bst 6");
      ...
      for (i=0; i < 500; ++i)  {
         ...
         c(btest_find(root, items[i]) == nullptr, "stb_bst 5");
         #ifdef STB_DEBUG
         btest__validate(root, 1);
         #endif
         for (j=0; j <= i; ++j)
            c(btest_find(root, items[j]) == nullptr, "stb_bst 3");
         for (   ; j < 500; ++j)
            c(btest_find(root, items[j]) != nullptr, "stb_bst 4");
         free(t);
      }
   }
}
...
int main(int argc, char **argv)
{
   ...
   {
      int i;
      char **s = stb_readdir_recursive("/sean", nullptr);
   ...
   c(1==stb_lex(mt, "this is a foo in the middle",nullptr), "stb_matcher_match 1");
   c(0==stb_lex(mt, "this is a bar in the middle",nullptr), "stb_matcher_match 2");
   c(0==stb_lex(mt, "this is a baz in the middle",nullptr), "stb_matcher_match 3");
   c(2==stb_lex(mt, "this is a bar",nullptr), "stb_matcher_match 4");
   c(0==stb_lex(mt, "this is a baz",nullptr), "stb_matcher_match 5");
   c(3==stb_lex(mt, "baz",nullptr), "stb_matcher_match 6");
   c(4==stb_lex(mt, "1_2_3_4",nullptr), "stb_matcher_match 7");
   c(0==stb_lex(mt, "1  3  3 3 3  2 ",nullptr), "stb_matcher_match 8");
   c(4==stb_lex(mt, "1  3  3 3 2  3 ",nullptr), "stb_matcher_match 9");
   c(5==stb_lex(mt, "C:/sean/prj/old/gdmag/mipmap/hqp/adol-c/CVS/Repository",nullptr), "stb_matcher_match 10");
   stb_matcher_free(mt);
   ...
   if (!f) { stb_fatal("Error: couldn't open file just written\n"); }
      ...
      if (p == nullptr) stb_fatal("Error: stb_file() failed");
      ...
      n2 = 0;
      while (s[n2]) ++n2;
      c(n1 == n2, "stb_stringfile():n length matches the non-nullptr strings");
      if (n2 == 3) {
   ...
   s = stb_tokens_allowempty("foo == ba*r", "#=*", nullptr);
   ...
   s = stb_tokens_stripwhite("foo == ba*r", "#=*", nullptr);
   ...
   s = stb_tokens_quoted("foo =\"=\" ba*\"\"r \" foo\" bah    ", "#=*", nullptr);
...
void test_sha1(void)
{
   ...
   for (i=0; i < sizeof(sha1_tests) / sizeof(sha1_tests[0]); ++i) {
      ...
      for (h=0; h < len; ++h) {
         ...
         v[2] = 0;
         buffer[h] = (unsigned char) strtol(v, nullptr, 16);
      }
      ...
      for (h=0; h < 20; ++h) {
         ...
         v[2] = 0;
         res = digest[h] == strtol(v, nullptr, 16);
...
int stb_matcher_find_slow(stb_matcher *m, char *str)
{
   int result = 1;
   int i,j,y,z;
   uint16 *previous = nullptr;
   uint16 *current = nullptr;
   uint16 *temp;
...
stb_bitset **compute_follow(short ** productions, stb_bitset **first, int start)
{
   ...
   assert(symset);
   for (i=0; i < num_symbols; ++i)
      follow[i] = (i >= first_nonterm ? stb_bitset_new(0, symset) : nullptr);
...
int main(int argc, char **argv)
{
   ...
   for (i=0; i < NUM_WORK; ++i) {
      stb_work_reach(work_consumer, nullptr, nullptr, s);
   }
   ...
   // start the consumers
   stb_create_thread(tq1_consumer, nullptr);
   stb_create_thread(tq1_consumer, nullptr);
   stb_create_thread(tq1_consumer, nullptr);

   stb_create_thread(tq2_consumer, nullptr);

   stb_create_thread(tq3_consumer, nullptr);
   stb_create_thread(tq3_consumer, nullptr);
   stb_create_thread(tq3_consumer, nullptr);
   stb_create_thread(tq3_consumer, nullptr);
   stb_create_thread(tq3_consumer, nullptr);
   ...
   stb_create_thread(tq4_consumer, nullptr);
...
{
   int i,j,k;
   STB__ARR(stb_summary_tree *) ret=nullptr;
   STB__ARR(stb_summary_tree2 *) all=nullptr;
   ...
   stb__summarize_compute_targets(nullptr, t, reweight, 1);
