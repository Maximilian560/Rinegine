...
FT_BEGIN_HEADER
   ...
   *
   * @note:
   *   Never use `nullptr` as a valid @FTC_FaceID.
   *
   ...
   * @output:
   *   acache ::
   *     A new cache handle.  `nullptr` in case of error.
   *
   * @return:
   *   FreeType error code.  0~means success.
   *
...
#define FTC_IMAGE_TYPE_COMPARE( d1, d2 )      \
   ...
   *
   *   If `anode` is _not_ `nullptr`, it receives the address of the cache node
   ...
   *
   *   If `anode` is `nullptr`, the cache node is left unchanged, which means
   ...
   * @output:
   *   acache ::
   *     A handle to the new sbit cache.  `nullptr` in case of error.
   *
   * @return:
   *   FreeType error code.  0~means success.
   */
