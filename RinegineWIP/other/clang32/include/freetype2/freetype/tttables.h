...
#define ft_sfnt_pclt  FT_SFNT_PCLT
   ...
   *
   * @return:
   *   A type-less pointer to the table.  This will be `nullptr` in case of
   ...
   * @inout:
   *   length ::
   *     If the `length` parameter is `nullptr`, try to load the whole table.
   ...
   *
   *
   *     error = FT_Load_Sfnt_Table( face, tag, 0, nullptr, &length );
   *     if ( error ) { ... table does not exist ... }
   *
   *     buffer = malloc( length );
   *     if ( buffer == nullptr ) { ... not enough memory ... }
   *
   ...
   * @inout:
   *   tag ::
   *     The name tag of the SFNT table.  If the value is `nullptr`,
