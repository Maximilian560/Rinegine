...
v0.0.3 (WIP)
Исправлено:
* Исправлена функция `RG_CreateText(RG_Text&, wstring, POINT3D, double, bool, LINK_GUI_TYPE, bool)`. Теперь при включении центрирования (последний параметр) текст будет правильно центрироваться учитывая масштаб (4 параметр).
Добавлено:
