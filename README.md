...
v0.0.3 (WIP)  
Исправлено:
* Исправлена функция `void RG_CreateText(RG_Text&, wstring, POINT3D, double, bool, LINK_GUI_TYPE, bool)`. Теперь при включении центрирования (последний параметр) текст будет правильно центрироваться учитывая масштаб (4 параметр).
  
Добавлено:
* Добавлен ещё конструктор для класса RG_Array, `RG_Array(uint size, type a);` для инициализации всех элементов массива одним значением, a.

Изменено:
* Изменена функция `void RG_CreateText(RG_Text& txt, wstring text, RG_Object& obj)` на `void RG_CreateText(RG_Text& txt, wstring text, RG_Object& obj, double)`. Последний параметр отвечает за масштаб текста привязанного к объекту, по умолчанию равен 1. Изменяет масштаб относительно объекта к которому привязан (если у объекта масштаб 3 а у текста 2, то финальный масштаб текста будет равен 6).
* Изменена функция `int Rinegine_Start(RG_Functions,void(&)(),RG_SettingWindow&)` и тип `RG_Array<int(*)(int)> RG_Functions`. Функции логики интерфейса(локиций) теперь должны иметь вид `int func(int)`. В параметр  int передаётся последняя локация, вернее локация из которой эта локация была запущена
___
v0.0.2 
...
***Внимание!*** При подключении данного модуля, все вызовы без исключения изменятся на свою WIP версию. Большинство будет работать без изменения, но иногда могут встречаться баги!
