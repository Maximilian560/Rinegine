...
void convert_fastchunk_inplace(fast_chunk *fc)
{
   ...
   for (i=0; i < 16; ++i)
      num_blocks += fc->blockdata[i] != nullptr;
...
{
   int z;
   assert(fc == nullptr || (fc->refcount > 0 && fc->refcount < 64));
   if (fc == nullptr || fc->blockdata[segment] == nullptr) {
      for (z=0; z < 16; ++z) {
...
void build_chunk(int chunk_x, int chunk_y, fast_chunk *fc_table[4][4], raw_mesh *rm)
{
   ...
   assert(rm->mm.input.geometry == nullptr);
